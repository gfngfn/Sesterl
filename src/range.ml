...
type t = Dummy of string | Real of real

let pp ppf rng =
  match rng with
  | Dummy s -> Format.fprintf ppf "(%s)" s
  | Real r ->
      if r.start_line = r.last_line then
        Format.fprintf ppf "file '%s', line %d, characters %d-%d" r.file_name
          r.start_line r.start_column r.last_column
      else
        Format.fprintf ppf
...
let get_file_name (rng : t) =
  match rng with Dummy s -> Printf.sprintf "(%s)" s | Real r -> r.file_name

let get_start_line (rng : t) =
  match rng with Dummy _ -> 0 | Real r -> r.start_line
