...
%token<Range.t> LET REC AND IN LAMBDA IF THEN ELSE TRUE FALSE DO RECEIVE ACT END CASE OF TYPE VAL MODULE STRUCT SIGNATURE SIG WITH EXTERNAL INCLUDE IMPORT FREEZE NEW VIA
...
;
branchnew:
  | BAR; ident=ident; REVARROW; NEW; tag=UPPER; pat=patcons; ARROW; c=comp {
      BranchNew{
        binder      = ident;
        tag         = tag;
        pattern     = pat;
...
;
branchvia:
  | BAR; ident=ident REVARROW; VIA; e0=exprbot; tag=UPPER; pat=patcons; ARROW; c=comp {
      BranchVia{
        binder      = ident;
        token       = e0;
        tag         = tag;
