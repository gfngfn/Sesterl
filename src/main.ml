...
let build (fpath_in : string) (dir_out_spec : string option) (is_verbose : bool)
    (externals : string list) =
  catch_error (fun () ->
      let current_directory = Sys.getcwd () in
      let abspath_in = make_absolute_path current_directory fpath_in in
      let external_map =
        externals
        |> List.fold_left
             (fun map s ->
      ...
      let pkgs, absdir_out, absdir_test_out, doc_configs_opt =
        ...
        | Some "sest" ->
            ...
            let absdir_out =
              ...
              | Some dir_out ->
                  append_dir current_directory (RelativeDir dir_out)
            in
            (pkgs, absdir_out, absdir_out, None)
        | Some ext -> raise (ConfigError (UnrecognizableExtension ext))
        | _ ->
            ...
            let pkgconfigs, main_config =
              PackageLoader.main external_map absdir_in
            in
            let pkgs =
              pkgconfigs
              |> List.map (fun (_, config) ->
                     let requires_tests =
                       String.equal config.ConfigLoader.package_name
                         main_config.ConfigLoader.package_name
                     in
      ...
      let _, pkgoutsacc =
        pkgs
        |> List.fold_left
             (fun (tyenv, outsacc) pkg ->
               let pkgnameopt, auxmods, mainmod, testmods = pkg in
               let tyenv, auxouts, mainout, testouts =
                 PackageChecker.main ~is_verbose tyenv ~aux:auxmods
                   ~main:mainmod ~test:testmods
               in
               ( tyenv,
                 Alist.extend outsacc (pkgnameopt, auxouts, mainout, testouts)
               ))
             (tyenv, Alist.empty)
      in

      let spec = { module_name_output_spec = DottedCamels } in
      ...
      pkgoutsacc |> Alist.to_list
      |> List.fold_left
           (fun gmap (pkgnameopt, auxouts, mainout, testouts) ->
             doc_configs_opt
             |> Option.map (fun (absdir_in, doc_configs) ->
                    doc_configs
                    |> List.iter (fun doc_config ->
                           let ConfigLoader.Html =
                             doc_config.ConfigLoader.document_output_format
                           in
                           let absdir_doc_out =
                             append_dir absdir_in
                               doc_config.ConfigLoader.document_output_directory
                           in
                           Core.Unix.mkdir_p absdir_doc_out;
                           let abspath_doc_out =
                             match pkgnameopt with
                             | None ->
                                 append_path absdir_doc_out
                                   (RelativePath "doc.html")
                             | Some pkgname ->
                                 let relpath =
                                   Printf.sprintf "%s.html"
                                     (OutputIdentifier.output_space_to_snake
                                        pkgname)
                                 in
                                 append_path absdir_doc_out
             ...
             outs
             |> List.fold_left
                  (fun gmap (out, is_for_test) ->
                    let sname = out.PackageChecker.space_name in
                    let imod =
                      (out.PackageChecker.attribute, out.PackageChecker.bindings)
                    in
                    let absdir =
                      if is_for_test then absdir_test_out else absdir_out
                    in
                    OutputErlangCode.main spec absdir gmap
                      ~package_name:pkgnameopt ~module_name:sname imod)
                  gmap)
           gmap
      |> ignore;
...
let command_build =
  ...
  in
  let info : Term.info = Term.info "build" in
  (term, info)

let command_config =
  let open Cmdliner in
  let term : unit Term.t = Term.(const config $ arg_in) in
  let info : Term.info = Term.info "config" in
  (term, info)
...
let () =
  let open Cmdliner in
  let subcommands = [ command_build; command_config ] in
