...
type sig_record_map =
  ((signature_source * SigRecord.t) abstracted * space_name) SigRecordMap.t
...
let check_single ~(is_verbose : bool) ~(is_main_module : bool)
    (sigrmap : sig_record_map) (tyenv_before : Typeenv.t)
    (source : SourceLoader.loaded_module) :
    (signature_source * SigRecord.t) abstracted * single_output =
  ...
  let absmodsigopt =
    let tyenv_for_sig =
      if is_main_module then tyenv_before else tyenv_for_mod
    in
    let address = Address.root |> Address.append_member modnm in
    utsigopt
    |> Option.map (Typechecker.typecheck_signature ~address tyenv_for_sig)
  in
  let _, abssigr, sname, (modattr, ibinds) =
    Typechecker.main tyenv_for_mod modident absmodsigopt utmod
  in
  let out =
    {
      module_name = modnm;
...
let main ~(is_verbose : bool) (tyenv_before : Typeenv.t)
    ~aux:(auxmods : SourceLoader.loaded_module list)
    ~main:(mainmod : SourceLoader.loaded_module)
    ~test:(testmods : SourceLoader.loaded_module list) :
    Typeenv.t * single_output list * single_output * single_output list =
  let sigrmap, auxoutacc =
    auxmods
    |> List.fold_left
         (fun (sigrmap, auxoutacc) auxmod ->
           let abssigr, auxout =
             check_single ~is_verbose ~is_main_module:false sigrmap tyenv_before
               auxmod
           in
           let sigrmap =
  ...
  let _sigrmap, testoutacc =
    testmods
    |> List.fold_left
         (fun (sigrmap, testoutacc) testmod ->
           let abssigr, testout =
             check_single ~is_verbose ~is_main_module:false sigrmap tyenv_before
               testmod
           in
