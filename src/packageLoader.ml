...
let load_config absdir_in =
  ...
  | Some language_version ->
      if LanguageVersion.is_supported language_version then config
      else raise (ConfigError (UnsupportedLanguageVersion language_version))
...
let main (external_map : external_map) (absdir : absolute_dir) :
    (absolute_dir * ConfigLoader.config) list * ConfigLoader.config =
  let rec aux ~(requires_test_deps : bool) (state : reading_state)
      (vertex : FileDependencyGraph.vertex) (absdir : absolute_dir) :
      ConfigLoader.config * reading_state =
    ...
    | None ->
        let loaded_dirs =
          state.loaded_dirs |> PackageDirMap.add absdir config
        in
        let loaded_names =
          state.loaded_names |> PackageNameMap.add pkgname absdir
        in
        let state = { state with loaded_dirs; loaded_names } in
        let state =
          let deps =
            if requires_test_deps then
              List.append config.ConfigLoader.dependencies
                config.ConfigLoader.test_dependencies
            else config.ConfigLoader.dependencies
          in
          deps
          |> List.fold_left
               (fun state dependency ->
                 ...
                 let absdir_sub =
                   ...
                   | ConfigLoader.Local absdir_sub -> absdir_sub
                   | ConfigLoader.Git _git_spec -> (
                       match
                         external_map |> ExternalMap.find_opt pkgname_sub
                       with
                       | None ->
                           raise
                             (PackageError
                                (NotFoundInExternalMap
                                   (pkgname_sub, external_map)))
                       | Some absdir_sub -> absdir_sub)
                 in
                 let absdir_sub =
                   match canonicalize_path absdir_sub with
                   | None ->
                       raise (PackageError (PackageDirNotFound absdir_sub))
                   | Some absdir -> absdir
                 in
                 ...
                 | None ->
                     ...
                     let graph, vertex_sub =
                       graph |> FileDependencyGraph.add_vertex absdir_sub
                     in
                     let graph =
                       graph
                       |> FileDependencyGraph.add_edge ~depending:vertex
                     ...
                     in
                     let _, state =
                       aux ~requires_test_deps:false { state with graph }
                         vertex_sub absdir_sub
                     in
                     state)
               state
        in
  ...
  let config, state =
    let graph, vertex =
      FileDependencyGraph.empty |> FileDependencyGraph.add_vertex absdir
    in
    let state =
      {
        graph;
