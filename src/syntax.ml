...
type module_name_output_spec = SingleSnake | DottedCamels

type output_spec = { module_name_output_spec : module_name_output_spec }
...
type ('a, 'b) pure_or_effectful = Pure of 'a | Effectful of 'b
[@@deriving show { with_path = false }]

type 'a cycle = Loop of 'a | Cycle of 'a List2.t

type 'a ranged = Range.t * 'a

let pp_ranged ppsub ppf (_, x) = Format.fprintf ppf "%a" ppsub x

type identifier = string

type type_name = string [@@deriving show { with_path = false }]

type kind_name = string [@@deriving show { with_path = false }]

type constructor_name = string [@@deriving show { with_path = false }]

type type_variable_name = string [@@deriving show { with_path = false }]

type row_variable_name = string [@@deriving show { with_path = false }]

type module_name = string [@@deriving show { with_path = false }]

type signature_name = string [@@deriving show { with_path = false }]
...
with type key = string = struct
  module Impl = Map.Make (String)
  include Impl

  let pp ppsub ppf labmap =
    labmap
    |> Impl.iter (fun label v -> Format.fprintf ppf "%s -> %a; " label ppsub v)
end
...
with type elt = label = struct
  module Impl = Set.Make (String)
  include Impl

  let pp ppf labset =
    labset |> Impl.iter (fun label -> Format.fprintf ppf "%s,@ " label)
end

let pp_identifier ppf s = Format.fprintf ppf "\"%s\"" s

let pp_uchar ppf uchar = Format.fprintf ppf "U+%X" (Uchar.to_int uchar)

type module_name_chain = module_name ranged * module_name ranged list
[@@deriving show { with_path = false }]
...
type manual_kind = manual_kind_main ranged

and manual_kind_main = MKind of manual_base_kind list * manual_base_kind

and manual_base_kind = manual_base_kind_main ranged

and manual_base_kind_main = MKindName of kind_name
...
and manual_row =
  | MRow of
      (label ranged * manual_type) list * (Range.t * row_variable_name) option

and binder = untyped_pattern * manual_type option

and constructor_branch =
  | ConstructorBranch of
      attribute list * constructor_name ranged * manual_type list

and synonym_or_variant =
  | BindSynonym of manual_type
  | BindVariant of constructor_branch list

and untyped_ast = untyped_ast_main ranged
...
and untyped_computation_ast = untyped_computation_ast_main ranged

and untyped_computation_ast_main =
  | CompDo of binder * untyped_computation_ast * untyped_computation_ast
  | CompReceive of
      untyped_receive_branch list
      * (untyped_ast * untyped_computation_ast) option
...
and type_variable_binder = type_variable_name ranged * manual_base_kind option
...
and pure_return = manual_type option * untyped_ast
...
and untyped_case_branch = CaseBranch of untyped_pattern * untyped_ast
...
and untyped_pattern_main =
  ...
  | PConstructor of
      module_name ranged list * constructor_name * untyped_pattern list
[@@deriving show { with_path = false }]

and untyped_module = untyped_module_main ranged
...
and untyped_binding = untyped_binding_main ranged
...
and untyped_signature = untyped_signature_main ranged
...
and untyped_declaration = untyped_declaration_main ranged

and untyped_declaration_main =
  | DeclVal of
      identifier ranged
      * type_variable_binder list
      * (row_variable_name ranged * label ranged list) list
      * manual_type
      * attribute list
...
and labeled_binder = label ranged * binder

and labeled_optional_binder = labeled_binder * untyped_ast option

and labeled_untyped_ast = label ranged * untyped_ast

and labeled_manual_type = label ranged * manual_type
[@@deriving show { with_path = false }]

and attribute = Attribute of (string * untyped_ast option) ranged

type attribute_warning = { position : Range.t; tag : string; message : string }
...
module BoundBothID = struct
  type t = Type of BoundID.t | Row of BoundRowID.t
...
type space_name = OutputIdentifier.space [@@deriving show { with_path = false }]

type local_name = OutputIdentifier.local [@@deriving show { with_path = false }]
...
type name = OutputIdentifier.t [@@deriving show { with_path = false }]
...
let stringify_opaque_id_quantifier qt =
  OpaqueIDMap.fold
    (fun oid pkd acc ->
      Alist.extend acc
        (Format.asprintf "%a" (TypeID.pp ~seen_from:Address.root) oid))
    qt Alist.empty
  |> Alist.to_list
  |> List.map (fun s -> " " ^ s)
  |> String.concat ","
