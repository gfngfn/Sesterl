...
module rec MonoTypeVarUpdatable : sig
  ...
  let pp ppf mtv = Format.fprintf ppf "%s" (show_mono_type_var_updatable mtv)
...
and MonoType : sig
  ...
  let equal (ty1 : t) (ty2 : t) : bool =
    let rec aux ((_, tymain1) : t) ((_, tymain2) : t) : bool =
      ...
      | ListType ty1, ListType ty2 -> aux ty1 ty2
    ...
    and aux_pid (Pid ty1) (Pid ty2) = aux ty1 ty2
    and aux_effect (Effect ty1) (Effect ty2) = aux ty1 ty2 in
...
let mono_type_var_updatable_witness : mono_type_var_updatable Alcotest.testable
    =
  (module MonoTypeVarUpdatable : Alcotest.TESTABLE
    with type t = mono_type_var_updatable)

let test_unify (ty1, ty2, assoc) () =
  Typechecker.unify ty1 ty2;
  assoc
  |> List.iter (fun (fid, mtvu_nonref) ->
         Alcotest.check mono_type_var_updatable_witness "free variable" !fid
           mtvu_nonref)
...
let () =
  ...
  run "Typechecker"
    [
      ( "unify",
        List.map
          (fun tuple -> test_case "unify" `Quick (test_unify tuple))
          [
