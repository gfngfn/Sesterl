...
let ( ==> ) (key : string) (v : value) = (key, v)

let keyed (key : string) (vs : value list) = Keyed (key, vs)

let relative_dir_to_string (RelativeDir s : relative_dir) : value = String s
...
let make (config : ConfigLoader.config) : assoc =
  let entry_plugins =
    ...
    ==> Assoc
          [
            ...
            ==> keyed "git" [ String Constants.plugin_url; v_git_spec ];
          ]
  in
  ...
  let entry_eunit_tests =
    ...
    ==> List
          (reldirs
          |> List.map (fun reldir ->
                 Keyed ("dir", [ relative_dir_to_string reldir ])))
  in
  let entry_deps =
    let deps_sesterl =
      config.dependencies
    ...
    in
    let deps_erlang =
      config.erlang_config.erlang_dependencies
      |> List.map (fun erldep ->
             let name = erldep.ConfigLoader.erlang_library_name in
             let v_dep =
               match erldep.ConfigLoader.erlang_library_source with
               | ErlangLibFromHex { version } -> String version
               | ErlangLibFromGit { repository = uri; git_spec } ->
                   let v_git_spec = make_git_spec git_spec in
  ...
  let entry_profile =
    ...
    in
    "profiles"
    ==> Assoc [ "test" ==> Assoc [ "deps" ==> Assoc test_deps_sesterl ] ]
  in
  let entries_relx =
                ...
                [
                  Keyed
                    ( "release",
                      [
                        Keyed
                          (release.relx_name, [ String release.relx_version ]);
                        List
                          (release.relx_applications
                          |> List.map (fun app -> Atom app));
                      ] );
