...
let succeed (a : 'a) : 'a t = fun _ -> Ok a

let failure (msg : string) : 'a t = fun _ -> Error (OtherMessage msg)

let bind (d : 'a t) (df : 'a -> 'b t) : 'b t =
 fun yval -> match d yval with Ok a -> df a yval | Error _ as e -> e

let ( >>= ) = bind

let get_scheme (field : string) (d : 'a t) (k : unit -> ('a, error) result) :
    'a t =
  let open ResultMonad in
  function
  | `O keyvals -> (
      match
        List.find_map
          (fun (k, v) -> if String.equal k field then Some v else None)
          keyvals
      with
      | None -> k ()
      | Some v -> d v)
...
let get_opt (field : string) (d : 'a t) : 'a option t =
  let d_some = d >>= fun v -> succeed (Some v) in
  let open ResultMonad in
...
let number : float t =
  let open ResultMonad in
  function `Float x -> return x | _ -> err NotAFloat

let string : string t =
  let open ResultMonad in
  function `String x -> return x | _ -> err NotAString

let bool : bool t =
  let open ResultMonad in
  function `Bool x -> return x | _ -> err NotABool
...
let list (d : 'a t) : 'a list t =
  ...
  | `A yvals ->
      ...
      |> List.fold_left
           (fun res yval ->
             res >>= fun acc ->
             d yval >>= fun a -> return (Alist.extend acc a))
           (return Alist.empty)
      >>= fun acc -> return (Alist.to_list acc)
  | _ -> err NotAnArray

type 'a branch = string * 'a t

let branch (field : string) (branches : 'a branch list)
    ~on_error:(errorf : string -> string) : 'a t =
  get field string >>= fun tag_gotten ->
  match
    branches
    |> List.find_map (fun (tag_candidate, d) ->
...
let map (f : 'a -> 'b) (d : 'a t) : 'b t =
  let open ResultMonad in
  fun yval -> d yval >>= fun a -> return (f a)

let map2 (f : 'a1 -> 'a2 -> 'b) (d1 : 'a1 t) (d2 : 'a2 t) : 'b t =
  let open ResultMonad in
  fun yval ->
    d1 yval >>= fun a1 ->
    d2 yval >>= fun a2 -> return (f a1 a2)

let map3 (f : 'a1 -> 'a2 -> 'a3 -> 'b) (d1 : 'a1 t) (d2 : 'a2 t) (d3 : 'a3 t) :
    'b t =
  let open ResultMonad in
  fun yval ->
    d1 yval >>= fun a1 ->
    d2 yval >>= fun a2 ->
    d3 yval >>= fun a3 -> return (f a1 a2 a3)
