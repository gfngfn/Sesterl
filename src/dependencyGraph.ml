...
type t = { labels : (data * GraphImpl.V.t) IDMap.t; main : GraphImpl.t }

let empty : t = { labels = IDMap.empty; main = GraphImpl.empty }
...
let get_vertex_token (map : (data * GraphImpl.V.t) IDMap.t) (tynm : type_name) :
    GraphImpl.V.t =
  match map |> IDMap.find_opt tynm with
  | None -> assert false
  | Some (_, vertex) -> vertex

let add_edge ~depended:(tynm1 : type_name) ~depending:(tynm2 : type_name)
    (graph : t) : t =
...
let topological_sort (graph : t) :
    ((type_name * data) list, type_name ranged cycle) result =
  match find_loop graph.main with
  | Some v -> Error (Loop (extract_vertex_error_info graph v))
  | None -> (
      let sccs = ComponentImpl.scc_list graph.main in
      match
        sccs
        |> List.find_map (fun scc ->
               match scc with
               | [] -> assert false
               | [ _ ] -> None
               | v1 :: v2 :: vrest ->
                   ...
                   Some
                     (Cycle (vs |> List2.map (extract_vertex_error_info graph))))
      with
