...
let report_config_error (e : config_error) : unit =
  ...
  | CyclicFileDependencyFound cycle -> (
      ...
      | Cycle abspaths ->
          ...
          abspaths |> List2.to_list
          |> List.iter (fun abspath -> Format.printf "  - '%s'\n" abspath))
  | MultipleModuleOfTheSameName (modnm, abspath1, abspath2) ->
      Format.printf
        "multiple module bound with the same name '%s':\n  - %s\n  - %s\n" modnm
        abspath1 abspath2
  | ModuleNotFound (rng, modnm) ->
      Format.printf "%a: module '%s' not found\n" Range.pp rng modnm
  | InvalidPackageName s -> Format.printf "invalid package name '%s'\n" s
  ...
  | NoOutputSpecForSingleSource ->
      Format.printf
        "no output spec ('--output' or '-o') for single source file\n"
  | UnsupportedLanguageVersion language_version ->
      Format.printf
        "unsupported language version '%s' (the version of this compiler is \
         '%s')\n"
        language_version Constants.semantic_version
...
let report_package_error (e : package_error) : unit =
  ...
  | DuplicatedPackageName (pkgname, abspath1, abspath2) ->
      Format.printf
        "multiple package have the same name '%s':\n  - %s\n  - %s\n" pkgname
        abspath1 abspath2
  ...
  | NotFoundInExternalMap (pkgname, external_map) ->
      ...
      knowns
      |> List.iter (fun (name, path) -> Format.printf "  - %s (%s)\n" name path)

let pp_type_parameter_list dispmap ppf bids =
  match bids with
  | [] -> ()
  | _ :: _ ->
      let pp_bound_id ppf bid =
        Format.fprintf ppf "%s" (dispmap |> DisplayMap.find_bound_id bid)
      in
...
let make_display_map_from_mono_types =
  DisplayMap.empty
  |> List.fold_left (fun dispmap ty -> dispmap |> TypeConv.collect_ids_mono ty)

let make_display_map_from_poly_types =
  DisplayMap.empty
  |> List.fold_left (fun dispmap pty ->
         dispmap |> TypeConv.collect_ids_poly pty)

let print_free_rows_and_base_kinds (dispmap : DisplayMap.t) =
  let row_names =
    dispmap
    |> DisplayMap.fold_free_row_id
         (fun frid (row_name, labset) acc ->
...
let report_unification_error ~actual:(ty1 : mono_type)
    ~expected:(ty2 : mono_type) (e : unification_error) : unit =
  match e with
  | Contradiction ->
  ...
  | Inclusion fid ->
      let dispmap = make_display_map_from_mono_types [ ty1; ty2 ] in
      let rng1, _ = ty1 in
      Format.printf "%a:" Range.pp rng1;
      Format.printf "  this expression has type\n";
      Format.printf "    %a\n" (TypeConv.pp_mono_type dispmap) ty1;
      Format.printf "  and type\n";
      Format.printf "    %a\n" (TypeConv.pp_mono_type dispmap) ty2;
      Format.printf
        "  at the same time, but these types are inconsistent as to the \
         occurrence of type variable %s\n"
  ...
  | InsufficientRowConstraint r ->
      ...
      print_free_rows_and_base_kinds dispmap;
      Format.printf
        "  The row parameter %a is specified so that it does not contain the \
         following label(s):\n"
...
let report_type_error (e : type_error) : unit =
  ...
  | InvalidNumberOfConstructorArguments (rng, ctor, len_expected, len_actual) ->
      Format.printf "%a:\n" Range.pp rng;
      Format.printf
        "  constructor '%s' expects %d argument(s), but is here applied to %d \
         argument(s)\n"
        ctor len_expected len_actual
  ...
  | InvalidNumberOfTypeArguments (rng, tynm, len_expected, len_actual) ->
      Format.printf "%a:\n" Range.pp rng;
      Format.printf
        "  type constructor '%s' expects %d argument(s), but is here applied \
         to %d argument(s)\n"
        tynm len_expected len_actual
  | KindContradiction (rng, tynm, kd_expected, kd_actual) ->
      Format.printf "%a:\n" Range.pp rng;
      Format.printf
        "  type constructor '%s' has kind %s, but is expected of kind %s\n" tynm
  ...
  | NotASubtypeTypeDefinition (rng, tynm, _tentry1, _tentry2) ->
      Format.printf "%a:\n" Range.pp rng;
      Format.printf
        "  not a subtype; type '%s' cannot be encapsulated (TODO: detailed \
         explanation)\n"
        tynm
  | NotASubtypeConstructorDefinition (rng, ctornm, _centry1, _centry2) ->
      Format.printf "%a:\n" Range.pp rng;
      Format.printf
        "  not a subtype; constructor '%s' cannot be encapsulated (TODO: \
         detailed explanation)\n"
        ctornm
  | NotASubtypeVariant (rng, _vid1, _vid2, ctor) ->
      Format.printf "%a:\n" Range.pp rng;
      Format.printf
        "  not a subtype about constructor '%s' (TODO: detailed explanation)\n"
        ctor
  | OpaqueIDExtrudesScopeViaValue (rng, _pty) ->
      Format.printf "%a:\n" Range.pp rng;
      Format.printf
        "  an abstract type extrudes its scope via value (TODO: detailed \
         explanation)\n"
  | OpaqueIDExtrudesScopeViaType (rng, _tyopac) ->
      Format.printf "%a:\n" Range.pp rng;
      Format.printf
        "  an abstract type extrudes its scope via type (TODO: detailed \
         explanation)\n"
  | OpaqueIDExtrudesScopeViaSignature (rng, _absmodsig) ->
      Format.printf "%a:\n" Range.pp rng;
      Format.printf
        "  an abstract type extrudes its scope via signature (TODO: detailed \
         explanation)\n"
  ...
  | BadArityOfOrderedArguments info ->
      Format.printf "%a:\n" Range.pp info.range;
      Format.printf
        "  the function expects %d ordered argument(s), but is applied to %d \
         ordered argument(s) here\n"
