...
let make_value (prefix : string) (i : int) =
  let rec aux chs i =
    ...
    if q <= 0 then ch :: chs else aux (ch :: chs) r
  in
  let chs = aux [] i in
...
let add_free_id fid dispmap =
  ...
  if fids |> FreeIDMap.mem fid then dispmap
  else
    let i = dispmap.current_max in
    let s = make_value "'" i in
    { dispmap with current_max = i + 1; free_ids = fids |> FreeIDMap.add fid s }

let add_free_row_id frid labset dispmap =
  let frids = dispmap.free_row_ids in
  if frids |> FreeRowIDMap.mem frid then dispmap
  else
    let i = dispmap.current_max in
    let s = make_value "?'" i in
    {
...
let add_bound_id bid dispmap =
  ...
  if bids |> BoundIDMap.mem bid then dispmap
  else
    let i = dispmap.current_max in
    let s = make_value "#" i in
    {
...
let add_bound_row_id brid labset dispmap =
  ...
  if brids |> BoundRowIDMap.mem brid then dispmap
  else
    let i = dispmap.current_max in
    let s = make_value "?#" i in
    {
...
let make_free_id_hash_set dispmap =
  ...
  dispmap.free_ids
  |> FreeIDMap.iter (fun fid _ -> FreeIDHashTable.add fidht fid ());
  fidht

let make_free_row_id_hash_set dispmap =
  let fridht = FreeRowIDHashTable.create 32 in
  dispmap.free_row_ids
  |> FreeRowIDMap.iter (fun frid (_, labset) ->
...
let make_bound_id_hash_set dispmap =
  ...
  dispmap.bound_ids
  |> BoundIDMap.iter (fun bid _ -> BoundIDHashTable.add bidht bid ());
  bidht

let make_bound_row_id_hash_set dispmap =
  let bridht = BoundRowIDHashTable.create 32 in
  dispmap.bound_row_ids
  |> BoundRowIDMap.iter (fun brid (_, labset) ->
         BoundRowIDHashTable.add bridht brid labset);
  bridht

let fold_free_id f acc dispmap = FreeIDMap.fold f dispmap.free_ids acc

let fold_free_row_id f acc dispmap =
  FreeRowIDMap.fold f dispmap.free_row_ids acc

let fold_bound_id f acc dispmap = BoundIDMap.fold f dispmap.bound_ids acc
