...
let pp_binder ppf (_, s) = pp_identifier ppf s
...
type declaration = ValDecl of bool * binder * untyped_ast
[@@deriving show { with_path = false }]
...
and 'a effect = Effect of 'a typ

and 'a pid_type = Pid of 'a typ
...
let lift_scheme rngf pred ty =
  ...
  let rec aux (rng, tymain) =
    match tymain with
    | BaseType bty -> (rngf rng, BaseType bty)
    ...
    | EffType (eff, ty0) -> (rngf rng, EffType (aux_effect eff, aux ty0))
    | PidType pidty -> (rngf rng, PidType (aux_pid_type pidty))
...
let lift ty = lift_scheme (fun rng -> rng) (fun _ -> false) ty
...
let instantiate lev pty =
  ...
  let rec aux (rng, ptymain) =
    ...
    | TypeVar (Mono mtv) -> (rng, TypeVar mtv)
    | TypeVar (Bound bid) ->
...
let overwrite_range_of_type (type a) (rng : Range.t) ((_, tymain) : a typ) :
    a typ =
  (rng, tymain)
...
let show_mono_type_var_ref = show_mono_type_var_scheme show_mono_type_scheme

let show_mono_type = show_mono_type_scheme show_mono_type_var_ref

let pp_mono_type ppf ty = Format.fprintf ppf "%s" (show_mono_type ty)

let show_poly_type_var = function
  | Bound bid -> Format.asprintf "%a" BoundID.pp bid
  | Mono tvref -> show_mono_type_var_ref tvref

let show_poly_type = show_mono_type_scheme show_poly_type_var

let pp_poly_type ppf pty = Format.fprintf ppf "%s" (show_poly_type pty)
