...
let primitive_module_name = "sesterl_internal_prim"

let decode_option_function = "decode_option"

let decode_option_function_with_default = "decode_option_with_default"
...
let frozen_type (rng : Range.t) ~rest:(tyrest : ('a, 'b) typ)
    ~receive:(tyrecv : ('a, 'b) typ) ~return:(tycod : ('a, 'b) typ) :
    ('a, 'b) typ =
...
let ( @-> ) tydoms tycod =
  let domain =
    { ordered = tydoms; mandatory = LabelAssoc.empty; optional = RowEmpty }
  in
...
let primitive_definitions =
  [
    {
      source = Some { identifier = "spawn"; typ = tyspawn };
      target =
        {
          target_name = "spawn";
          parameters = [ "F" ];
    ...
    {
      source = Some { identifier = "send"; typ = tysend };
      target =
        {
          target_name = "send";
          parameters = [ "Pid"; "Msg" ];
    ...
    {
      source = Some { identifier = "return"; typ = tyreturn };
      target = { target_name = "return"; parameters = [ "X" ]; code = "X" };
    };
    {
      source = Some { identifier = "self"; typ = tyself };
      target = { target_name = "self"; parameters = []; code = "erlang:self()" };
    };
    {
      source = Some { identifier = "print_debug"; typ = typrintdebug };
      target =
        {
    ...
    {
      source = Some { identifier = "format"; typ = tyformat };
      target =
        {
          target_name = "format";
          parameters = [ "{Fmt, _Arity}"; "Arg" ];
          code =
            "Args = case Arg of ok -> []; _ -> tuple_to_list(Arg) end, \
      ...
      target =
        {
          ...
          code =
            "case maps:find(Key, Options) of error -> Thunk(); {ok, Value} -> \
             Value end";
        };
    };
  ]
...
let add_variant_types
    (vntdefs :
      (type_name * TypeID.t * BoundID.t list * constructor_definition list) list)
    (tyenv, gmap) =
  let tyenv : Typeenv.t =
    vntdefs
    |> List.fold_left
         (fun tyenv vntdef ->
           ...
           let centryacc, ctormap =
             ...
             |> List.fold_left
                  (fun (centryacc, ctormap) ctordef ->
                    ...
                    let ctormap =
                      ctormap |> ConstructorMap.add ctornm (ctorid, paramtys)
                    in
                    (centryacc, ctormap))
           ...
           let tyenv =
             centryacc |> Alist.to_list
             |> List.fold_left
                  (fun tyenv (ctornm, centry) ->
...
let add_operators (ops : (string * poly_type * string) list)
    ((tyenv, nmap) : Typeenv.t * name_map) : Typeenv.t * name_map =
  let tyenv =
    ops
    |> List.fold_left
         (fun tyenv (x, pty, target) ->
           let name =
             OutputIdentifier.Operator (OutputIdentifier.operator target)
           in
...
let add_primitives (prims : primitive_definition list)
    ((tyenv, nmap) : Typeenv.t * name_map) : Typeenv.t * name_map =
  prims
  |> List.fold_left
       (fun (tyenv, nmap) primdef ->
         ...
         | Some srcdef ->
             ...
             let gname =
               ...
               match
                 OutputIdentifier.generate_global targetdef.target_name
                   ~suffix:"" ~arity ~has_option:false
               with
               | None -> assert false
               | Some gname -> gname
             in
             let tyenv =
               tyenv
               |> Typeenv.add_value srcdef.identifier srcdef.typ
