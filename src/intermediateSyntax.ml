...
and branch = IBranch of pattern * ast

let pp_sep_comma ppf () = Format.fprintf ppf ",@ "

let rec pp_val_binding_sub ppf (gname, e) =
  Format.fprintf ppf "%a =@[<hov>@ %a@]@," OutputIdentifier.pp_global gname
    pp_ast e

and pp_val_binding ppf = function
  | INonRec (_, gname, _, e) ->
      Format.fprintf ppf "val %a" pp_val_binding_sub (gname, e)
  | IRec recbinds ->
      let pairs = recbinds |> List.map (fun (_, gname, _, e) -> (gname, e)) in
      Format.fprintf ppf "val %a"
        (Format.pp_print_list ~pp_sep:pp_sep_comma pp_val_binding_sub)
        pairs
  | IExternal (gname, code) ->
      Format.fprintf ppf "val %a = external@ \"%s\"@,"
        OutputIdentifier.pp_global gname code

and pp_binding ppf = function
  | IBindVal valbind -> pp_val_binding ppf valbind
  | IBindModule (sname, _modattr, ibinds) ->
      Format.fprintf ppf "module %a = @[<v2>{%a}@]@," OutputIdentifier.pp_space
        sname
        (Format.pp_print_list pp_binding)
        ibinds

and pp_ast ppf = function
  | IBaseConst bc -> pp_base_constant ppf bc
  | IVar name -> OutputIdentifier.pp ppf name
  | ILambda (lnamerecopt, ordipats, mndipatmap, optipatmap, e) ->
      let snamerec =
        match lnamerecopt with
        | Some lnamerec ->
            Format.asprintf "%a" OutputIdentifier.pp_local lnamerec
        | None -> ""
      in
      Format.fprintf ppf "\\%s(%a -{%a} ?{%a}) ->@[<hov2>@ %a@]" snamerec
        (Format.pp_print_list ~pp_sep:pp_sep_comma pp_pattern)
        ordipats (LabelAssoc.pp pp_pattern) mndipatmap
        (LabelAssoc.pp (fun ppf (ipat, astopt) ->
             match astopt with
             | None -> Format.fprintf ppf "%a" pp_pattern ipat
             | Some ast ->
                 Format.fprintf ppf "%a = %a" pp_pattern ipat pp_ast ast))
        optipatmap pp_ast e
  | IApply (name, _, eargs, mndargmap, optargmap) ->
      Format.fprintf ppf "%a@[<hov2>(%a -{%a} ?{%a})@]" OutputIdentifier.pp name
        (Format.pp_print_list ~pp_sep:pp_sep_comma pp_ast)
        eargs (LabelAssoc.pp pp_ast) mndargmap (LabelAssoc.pp pp_ast) optargmap
  | ILetIn (lname, e1, e2) ->
      Format.fprintf ppf "(let %a =@[<hov2>@ %a@]@ in@ %a)"
        OutputIdentifier.pp_local lname pp_ast e1 pp_ast e2
  | ICase (e0, ibrs) ->
      Format.fprintf ppf "(case@[<hov2>@ %a@]@ of@[<hov2>@ %a@]@ end)" pp_ast e0
        (Format.pp_print_list pp_branch)
        ibrs
  | ITuple es ->
      Format.fprintf ppf "{%a}"
        (Format.pp_print_list ~pp_sep:pp_sep_comma pp_ast)
        (es |> TupleList.to_list)
  | _ -> Format.fprintf ppf "..."

and pp_branch ppf = function
  | IBranch (ipat, e) ->
      Format.fprintf ppf "%a ->@[<hov2>@ %a@];@ " pp_pattern ipat pp_ast e
