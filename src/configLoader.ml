...
type git_spec = Tag of string | Ref of string | Branch of string
...
type relx = { relx_release : relx_release; relx_dev_mode : bool }
...
type document_output_format = Html
...
let git_spec_decoder : git_spec YamlDecoder.t =
  ...
  branch "type"
    [
      "tag" ==> (get "value" string >>= fun tag -> succeed (Tag tag));
      "ref" ==> (get "value" string >>= fun hash -> succeed (Ref hash));
      "branch" ==> (get "value" string >>= fun branch -> succeed (Branch branch));
    ]
    ~on_error:(fun other ->
      Printf.sprintf "unsupported type '%s' for specifying sources from Git"
        other)
  ...
  branch "type"
    [
      ...
      ==> ( get "version" string >>= fun version ->
            succeed (ErlangLibFromHex { version }) );
      "git"
      ==> ( get "repository" string >>= fun repository ->
            get "spec" git_spec_decoder >>= fun git_spec ->
            succeed (ErlangLibFromGit { repository; git_spec }) );
    ]
    ~on_error:(fun other ->
      Printf.sprintf "unsupported type '%s' for specifying dependency sources"
        other)
...
let erlang_dependency_decoder : erlang_library YamlDecoder.t =
  ...
  succeed { erlang_library_name = name; erlang_library_source = erlsrc }
...
let relx_decoder : relx YamlDecoder.t =
  ...
  succeed { relx_release = release; relx_dev_mode = dev_mode }

let erlang_config_decoder : erlang_config YamlDecoder.t =
  let open YamlDecoder in
  get_or_else "output_directory" string Constants.default_output_directory
  >>= fun reldir_out ->
  get_or_else "test_output_directory" string
    Constants.default_test_output_directory
  >>= fun reldir_test_out ->
  get_or_else "erlang_dependencies" (list erlang_dependency_decoder) []
  >>= fun erldeps ->
...
let source_decoder (confdir : absolute_dir) : dependency_source YamlDecoder.t =
  ...
  branch "type"
    [
      ...
      ==> ( get "repository" string >>= fun repository ->
            get "spec" git_spec_decoder >>= fun git_spec ->
            succeed (Git { repository; git_spec }) );
    ]
    ~on_error:(fun other ->
      Printf.sprintf "unsupported type '%s' for specifying dependency sources"
        other)
...
let dependency_decoder (confdir : absolute_dir) : dependency YamlDecoder.t =
  ...
  succeed { dependency_name = name; dependency_source = source }
...
let document_output_format_decoder : document_output_format YamlDecoder.t =
  ...
  branch "type"
    ...
    ~on_error:(fun other ->
      Printf.sprintf
        "unsupported type '%s' for specifying document output format" other)
...
let config_decoder (confdir : absolute_dir) : config YamlDecoder.t =
  ...
  get_or_else "dependencies" (list (dependency_decoder confdir)) []
  >>= fun dependencies ->
  get_or_else "test_dependencies" (list (dependency_decoder confdir)) []
  >>= fun test_dependencies ->
  get_or_else "document_outputs" (list document_output_decoder) []
  >>= fun document_outputs ->
  get_or_else "erlang" erlang_config_decoder default_erlang_config
  >>= fun erlang_config ->
  let config =
    {
      ...
      main_module_name;
      source_directories =
        srcdirs |> List.map (fun srcdir -> RelativeDir srcdir);
      test_directories =
        testdirs |> List.map (fun testdir -> RelativeDir testdir);
      document_outputs;
      dependencies;
      test_dependencies;
      erlang_config;
