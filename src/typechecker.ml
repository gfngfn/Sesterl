...
type variant_definition =
  type_name * TypeID.t * BoundID.t list * constructor_map

type rec_morph = MonoRec of mono_type | PolyRec of poly_type
...
module SynonymNameHashSet = Hashtbl.Make (struct
  type t = type_name
...
let raise_error e = raise (TypeError e)
...
let internbidf (bidmap : BoundID.t BoundIDMap.t) (bid1 : BoundID.t)
    (pty2 : poly_type) : bool =
  match pty2 with
...
let internbridf (_bidmap : BoundID.t BoundIDMap.t) (_brid1 : BoundRowID.t)
    (_nomrow2 : normalized_poly_row) : bool =
  (* TODO: implement this when type definitions become able to take row parameters *)
  false

let add_dummy_fold (tynm : type_name) (tyid : TypeID.t) (bids : BoundID.t list)
    (ctormap : constructor_map) (sigr : SigRecord.t) : SigRecord.t =
  let bid = BoundID.fresh () in
  let dr = Range.dummy "add_dummy_fold" in
  let plabmap =
    ConstructorMap.fold
      (fun ctornm (_ctorid, ptyargs) plabmap ->
        let domty =
          {
            ordered = ptyargs;
        ...
        in
        plabmap
        |> LabelAssoc.add ctornm
             (dr, FuncType (domty, (dr, TypeVar (Bound bid)))))
  ...
  let domty =
    {
      ordered =
        [
          ( dr,
            TypeApp
              (tyid, bids |> List.map (fun bid -> (dr, TypeVar (Bound bid)))) );
        ];
      mandatory = plabmap;
      optional = RowEmpty;
    }
...
let add_constructor_definitions (ctordefs : variant_definition list)
    (sigr : SigRecord.t) : SigRecord.t =
  ctordefs
  |> List.fold_left
       (fun sigr ctordef ->
         ...
         let sigr =
           ConstructorMap.fold
             (fun ctornm (ctorid, ptyargs) sigr ->
               let centry =
                 {
                   belongs = tyid;
...
let make_type_scheme_from_constructor_entry (centry : constructor_entry) :
    type_scheme =
  let { belongs = tyid; type_variables = bids; parameter_types = ptys; _ } =
    centry
  in
  let dr = Range.dummy "make_type_scheme_from_constructor_entry" in
  let domty =
    { ordered = ptys; mandatory = LabelAssoc.empty; optional = RowEmpty }
  in
  let ty_cod =
    (dr, TypeApp (tyid, bids |> List.map (fun bid -> (dr, TypeVar (Bound bid)))))
  in
...
let make_address_module_list (address : Address.t) : module_name list =
  ...
  |> List.fold_left
       (fun opt adelem ->
         ...
         | _ -> None)
       (Some Alist.empty)
  |> Option.value ~default:Alist.empty
  |> Alist.to_list
...
let get_dependency_on_synonym_types (vertices : SynonymNameSet.t) (pre : pre)
    (mty : manual_type) : SynonymNameSet.t =
  ...
  let rec aux ((_, mtymain) : manual_type) : unit =
    ...
    | MFuncType ((mtydoms, mndlabmtys, mrow), mtycod) ->
        ...
        aux_row mrow;
        aux mtycod
    | MProductType mtys -> mtys |> TupleList.to_list |> List.iter aux
    | MRecordType mrow -> aux_row mrow
    ...
    | MPackType utsig -> aux_signature utsig
  ...
  and aux_row (mrow : manual_row) : unit =
    match mrow with MRow (optlabmtys, _) -> aux_labeled_list optlabmtys
  and aux_signature (utsig : untyped_signature) : unit =
    ()
    (* TODO: implement this or restrict the syntax of `pack` *)
  in
  aux mty;
  SynonymNameHashSet.fold
    (fun sid () set -> set |> SynonymNameSet.add sid)
...
let find_module_from_chain (tyenv : Typeenv.t)
    ((modident, projs) : module_name_chain) : module_entry =
  ...
  let ret, _ =
    ...
    |> List.fold_left
         (fun (mentry, rng) proj ->
           ...
           match modsig with
           | _, ConcFunctor _ -> raise_error (NotOfStructureType (rng, modsig))
           | _, ConcStructure sigr -> (
               let rngproj, modnm = proj in
               match sigr |> SigRecord.find_module modnm with
               | None -> raise_error (UnboundModuleName (rngproj, modnm))
               | Some mentry ->
                   let rng, _ = proj in
                   (mentry, rng)))
         (init, rng)
...
let update_type_environment_by_signature_record (sigr : SigRecord.t)
    (tyenv : Typeenv.t) : Typeenv.t =
  sigr
  |> SigRecord.fold
       ...
       ~t:(fun tynm tentry tyenv -> tyenv |> Typeenv.add_type tynm tentry)
       ~m:(fun modnm mentry tyenv -> tyenv |> Typeenv.add_module modnm mentry)
       ~s:(fun signm absmodsig -> Typeenv.add_signature signm absmodsig)
       tyenv

let add_open_specs_to_type_environment (openspecs : module_name_chain list)
    (tyenv : Typeenv.t) : Typeenv.t =
  openspecs
  |> List.fold_left
       (fun tyenv openspec ->
...
let iapply (efun : ast) (mrow : mono_row)
    ((eargs, mndargmap, optargmap) :
      ast list * ast LabelAssoc.t * ast LabelAssoc.t) : ast =
  match efun with
  | IVar name -> IApply (name, mrow, eargs, mndargmap, optargmap)
  | _ ->
      ...
      ILetIn
        ( lname,
          efun,
          IApply
            (OutputIdentifier.Local lname, mrow, eargs, mndargmap, optargmap) )

let ilambda
    ((ordipats, mndipatmap, optipatmap) :
      pattern list * pattern LabelAssoc.t * (pattern * ast option) LabelAssoc.t)
    (e0 : ast) : ast =
...
let iletrecin_single (_, _, name_outer, name_inner, e1) (e2 : ast) : ast =
  ...
  | ILambda (None, ordnames, mndnamemap, optnamemap, e0) ->
      ILetIn
        ( name_outer,
          ILambda (Some name_inner, ordnames, mndnamemap, optnamemap, e0),
          e2 )
  | _ -> assert false

let iletrecin_multiple
    (binds : (identifier * poly_type * local_name * local_name * ast) List2.t)
    (e2 : ast) : ast =
  ...
  let ipat_inner_tuple =
    IPTuple
      (binds |> TupleList.map (fun (_, _, _, name_inner, _) -> IPVar name_inner))
  in
  let name_for_whole_rec = OutputIdentifier.fresh () in
  let tuple_entries =
    binds
    |> TupleList.map (fun (_, _, _name_outer, _name_inner, e1) ->
           match e1 with
           | ILambda (None, ordnames, mndnamemap, optnamemap, e0) ->
               ILambda
                 ( None,
                   ...
                   iletpatin ipat_inner_tuple
                     (IApply
                        ( OutputIdentifier.Local name_for_whole_rec,
                          RowEmpty,
                          [],
                          LabelAssoc.empty,
                          LabelAssoc.empty ))
  ...
  let ipat_outer_tuple =
    IPTuple
      (binds |> TupleList.map (fun (_, _, name_outer, _, _) -> IPVar name_outer))
  in
  iletpatin ipat_outer_tuple
    (iapply
       (ILambda
          ( Some name_for_whole_rec,
            [],
            LabelAssoc.empty,
            LabelAssoc.empty,
            ITuple tuple_entries ))
       RowEmpty
       ([], LabelAssoc.empty, LabelAssoc.empty))
    e2

let iletrecin
    (binds : (identifier * poly_type * local_name * local_name * ast) list)
    (e2 : ast) : ast =
  match binds with
  | [] -> assert false
  | [ bind ] -> iletrecin_single bind e2
  | bind1 :: bind2 :: rest ->
      iletrecin_multiple (List2.make bind1 bind2 rest) e2
...
let occurs (fid : FreeID.t) (ty : mono_type) : bool =
  ...
  let rec aux ((_, tymain) : mono_type) : bool =
    ...
    | ProductType tys -> tys |> TupleList.to_list |> aux_list
    | RecordType row -> aux_row row
    | TypeApp (_tyid, tyargs) -> aux_list tyargs
    ...
    | TypeVar (Updatable { contents = Link ty }) -> aux ty
    | TypeVar (Updatable { contents = Free fidx }) ->
        if FreeID.equal fid fidx then true
        else (
...
*)
          false)
    | TypeVar (MustBeBound _) -> false
    | PackType _modsig -> false
  (* Signatures do not contain free IDs. *)
  and aux_domain (domain : mono_domain_type) : bool =
    let { ordered = tydoms; mandatory = mndlabmap; optional = optrow } =
      domain
    in
  ...
  and aux_pid_type (Pid ty) = aux ty
  and aux_row = function
    | RowCons (_, ty, row) ->
        let b1 = aux ty in
        let b2 = aux_row row in
        b1 || b2
    | RowVar (UpdatableRow { contents = FreeRow _ }) -> false
    | RowVar (UpdatableRow { contents = LinkRow row }) -> aux_row row
    | RowVar (MustBeBoundRow _) -> false
    | RowEmpty -> false
...
let occurs_row (frid : FreeRowID.t) (row : mono_row) : bool =
  ...
  and aux_row = function
    ...
    | RowVar (MustBeBoundRow _mbbrid) -> false
    | RowEmpty -> false
  and aux_label_assoc (labmap : mono_type LabelAssoc.t) =
    LabelAssoc.fold
      (fun _ ty bacc ->
...
let rec opaque_occurs_in_type_scheme :
          'a 'b.
          (quantifier -> TypeID.t -> bool) ->
          ('a -> bool) ->
          quantifier ->
          ('a, 'b) typ ->
          bool =
 fun tyidp tvp quant ->
  let rec aux (_, ptymain) =
    ...
    | FuncType (domain, tycod) -> aux_domain domain || aux tycod
    | TypeApp (tyid, tyargs) -> tyidp quant tyid || List.exists aux tyargs
    | RecordType row -> aux_row row
    | TypeVar tv -> tvp tv
    | PackType absmodsig ->
  ...
  and aux_pid = function Pid ty -> aux ty
  and aux_effect = function Effect ty -> aux ty
  and aux_row = function
    | RowCons (_, ty, row) ->
        let b1 = aux ty in
        let b2 = aux_row row in
...
and opaque_occurs_in_poly_type (quant : quantifier) : poly_type -> bool =
  let tvp : poly_type_var -> bool = function
    | Mono (Updatable { contents = Link ty }) ->
        opaque_occurs_in_mono_type quant ty
    | _ -> false
  in
  opaque_occurs_in_type_scheme opaque_occurs_in_type_id tvp quant
...
and opaque_occurs (quant : quantifier) (modsig : module_signature) : bool =
  match modsig with
  | _, ConcStructure sigr -> opaque_occurs_in_structure quant sigr
...
let label_assoc_union = LabelAssoc.union (fun _ _ ty2 -> Some ty2)
...
let generate_global_name ~(is_test_suite : bool) ~(arity : int)
    ~(has_option : bool) (rng : Range.t) (x : identifier) : global_name =
  let suffix = if is_test_suite then "_test_" else "" in
  match OutputIdentifier.generate_global x ~suffix ~arity ~has_option with
  | None -> raise_error (InvalidIdentifier (rng, x))
  | Some gname -> gname
...
let global_name_scheme (is_test_suite : bool) valbind =
  let arity =
    List.length valbind.vb_parameters + List.length valbind.vb_mandatories
  in
  let has_option = List.length valbind.vb_optionals > 0 in
  let rngv, x = valbind.vb_identifier in
  let gname = generate_global_name ~is_test_suite ~arity ~has_option rngv x in
  (gname, gname)

let types_of_format (lev : int) (fmtelems : format_element list) :
    mono_type list =
  fmtelems
  |> List.map (function
       | FormatHole (hole, _) ->
           ...
           let ty =
             match hole with
             | HoleC -> (rng, BaseType CharType)
             | HoleF | HoleE | HoleG -> (rng, BaseType FloatType)
             | HoleS -> (rng, BaseType BinaryType)
             | HoleP | HoleW -> fresh_type_variable lev rng
           in
           [ ty ]
       | FormatConst _ | FormatDQuote | FormatBreak | FormatTilde -> [])
  |> List.concat
...
let type_of_base_constant (lev : int) (rng : Range.t) (bc : base_constant) =
  ...
  | BinaryByString _ | BinaryByInts _ -> (rng, BaseType BinaryType)
  | String _ ->
      Primitives.list_type rng (Range.dummy "string_literal", BaseType CharType)
  ...
  | FormatString fmtelems ->
      let tyarg =
        ...
        | ty1 :: tys ->
            (Range.dummy "format", ProductType (TupleList.make ty1 tys))
      in
      Primitives.format_type rng tyarg

let rec unify_aux (ty1 : mono_type) (ty2 : mono_type) :
    (unit, unification_error) result =
  let open ResultMonad in
  let _, ty1main = ty1 in
  let _, ty2main = ty2 in
  match (ty1main, ty2main) with
  | TypeVar (Updatable { contents = Link ty1l }), _ -> unify_aux ty1l ty2
  | _, TypeVar (Updatable { contents = Link ty2l }) -> unify_aux ty1 ty2l
  | TypeVar (MustBeBound mbbid1), TypeVar (MustBeBound mbbid2) ->
      if MustBeBoundID.equal mbbid1 mbbid2 then return () else err Contradiction
  | TypeApp (tyid1, tyargs1), TypeApp (tyid2, tyargs2) ->
      if TypeID.equal tyid1 tyid2 then unify_aux_list tyargs1 tyargs2
      else err Contradiction
  | BaseType bt1, BaseType bt2 ->
      if bt1 = bt2 then return () else err Contradiction
  | FuncType (domain1, ty1cod), FuncType (domain2, ty2cod) ->
      unify_aux_domain domain1 domain2 >>= fun () -> unify_aux ty1cod ty2cod
  | EffType (domain1, eff1, tysub1), EffType (domain2, eff2, tysub2) ->
      unify_aux_domain domain1 domain2 >>= fun () ->
      unify_aux_effect eff1 eff2 >>= fun () -> unify_aux tysub1 tysub2
  | PidType pidty1, PidType pidty2 -> unify_aux_pid_type pidty1 pidty2
  | ProductType tys1, ProductType tys2 ->
      unify_aux_list (tys1 |> TupleList.to_list) (tys2 |> TupleList.to_list)
  | RecordType row1, RecordType row2 -> unify_aux_row row1 row2
  | PackType absmodsig1, PackType absmodsig2 -> (
      try
        subtype_abstract_with_abstract ~cause:(Range.dummy "unify1")
          ~address:Address.root absmodsig1 absmodsig2;
        subtype_abstract_with_abstract ~cause:(Range.dummy "unify2")
          ~address:Address.root absmodsig2 absmodsig1;
        return ()
      with _ -> err Contradiction)
  | ( TypeVar (Updatable ({ contents = Free fid1 } as mtvu1)),
      TypeVar (Updatable { contents = Free fid2 }) ) ->
      if FreeID.equal fid1 fid2 then return ()
      else (
        mtvu1 := Link ty2;
        return ())
...
and unify_aux_free_id_and_record (fid1 : FreeID.t)
    (mtvu1 : mono_type_var_updatable ref) (ty2 : mono_type) =
  let open ResultMonad in
  let b = occurs fid1 ty2 in
  if b then err @@ Inclusion fid1
  else (
    mtvu1 := Link ty2;
    return ())
...
and unify_aux_list tys1 tys2 =
  ...
  try
    List.fold_left2
      (fun res ty1 ty2 -> res >>= fun () -> unify_aux ty1 ty2)
      (return ()) tys1 tys2
  with Invalid_argument _ -> err Contradiction

and unify_aux_domain domain1 domain2 =
  let open ResultMonad in
  let { ordered = ty1doms; mandatory = mndlabmap1; optional = optrow1 } =
    domain1
  in
  let { ordered = ty2doms; mandatory = mndlabmap2; optional = optrow2 } =
    domain2
  in
...
and unify_aux_effect (Effect ty1) (Effect ty2) = unify_aux ty1 ty2

and unify_aux_pid_type (Pid ty1) (Pid ty2) = unify_aux ty1 ty2
...
and unify_aux_row (row1 : mono_row) (row2 : mono_row) =
  ...
  | ( RowVar (UpdatableRow ({ contents = FreeRow frid1 } as mtvu1)),
      RowVar (UpdatableRow { contents = FreeRow frid2 }) ) ->
      if FreeRowID.equal frid1 frid2 then return ()
      else
  ...
  | RowVar (UpdatableRow ({ contents = FreeRow frid1 } as mrvu1)), _ ->
      if occurs_row frid1 row2 then err @@ InclusionRow frid1
      else
        let labset1 = KindStore.get_free_row frid1 in
  ...
  | _, RowVar (UpdatableRow ({ contents = FreeRow frid2 } as mrvu2)) ->
      if occurs_row frid2 row1 then err @@ InclusionRow frid2
      else
        let labset2 = KindStore.get_free_row frid2 in
  ...
  | RowVar (MustBeBoundRow mbbrid1), RowVar (MustBeBoundRow mbbrid2) ->
      if MustBeBoundRowID.equal mbbrid1 mbbrid2 then return ()
      else err Contradiction
  ...
  | RowEmpty, RowEmpty -> return ()
  | RowEmpty, RowCons (_, _, _) -> err Contradiction
...
and unify_aux_label_assoc_exact labmap1 labmap2 =
  ...
  in
  LabelAssoc.fold
    (fun _ res resacc -> resacc >>= fun () -> res)
    merged (return ())
...
and unify_aux_label_assoc_intersection labmap1 labmap2 =
  ...
  in
  LabelAssoc.fold
    (fun label (ty1, ty2) res -> res >>= fun () -> unify_aux ty1 ty2)
    intersection (return ())
...
and solve_membership_aux (rng : Range.t) (label : label) (ty : mono_type)
    (row : mono_row) : (mono_row, unification_error) result =
  let open ResultMonad in
  match row with
  | RowCons ((rng0, label0), ty0, row0) ->
      if String.equal label0 label then
        unify_aux ty0 ty >>= fun () -> return row0
      else
  ...
  | RowVar (UpdatableRow ({ contents = FreeRow frid0 } as mrvu0)) ->
      ...
      if labset0 |> LabelSet.mem label then err Contradiction
  ...
  | RowVar (MustBeBoundRow _) ->
      err Contradiction
      (* TODO (error): solve_membership_aux, reject for must-be-bound row IDs *)
  | RowEmpty -> err Contradiction
(* TODO (error): solve_membership_aux, RowEmpty *)
...
and solve_disjointness_aux (row : mono_row) (labset : LabelSet.t) =
  ...
  | RowCons ((rng, label), ty, rowsub) ->
      if labset |> LabelSet.mem label then err Contradiction
      else solve_disjointness_aux rowsub labset
  ...
  | RowVar (MustBeBoundRow mbbrid0) ->
      let labset0 =
        KindStore.get_bound_row (MustBeBoundRowID.to_bound mbbrid0)
      in
      if LabelSet.subset labset labset0 then return ()
      else
        err
        @@ InsufficientRowConstraint
             { id = mbbrid0; given = labset0; required = labset }
  | RowEmpty -> return ()
...
and unify (tyact : mono_type) (tyexp : mono_type) : unit =
  ...
  | Error e ->
      raise_error
        (UnificationError { actual = tyact; expected = tyexp; detail = e })

and unify_effect (Effect tyact : mono_effect) (Effect tyexp : mono_effect) :
    unit =
  let res = unify_aux tyact tyexp in
  match res with
  | Ok () -> ()
...
and make_rec_initial_type_from_annotation (preL : pre)
    (letbind : untyped_let_binding) : pre * poly_type option =
  let rngv, x = letbind.vb_identifier in
...
and make_type_parameter_assoc (pre : pre) (tyvarnms : type_variable_binder list)
    : pre * type_parameter_assoc =
  tyvarnms
  |> List.fold_left
       (fun (pre, assoc) ((rng, tyvarnm), kdannot) ->
         ...
         | Some assoc ->
             let localtyparams =
               pre.local_type_parameters |> TypeParameterMap.add tyvarnm mbbid
             in
             let pre = { pre with local_type_parameters = localtyparams } in
             (pre, assoc))
       (pre, TypeParameterAssoc.empty)

and decode_manual_base_kind (pre : pre) ((rng, mnbkdmain) : manual_base_kind) :
    base_kind =
...
and decode_manual_type (pre : pre) (mty : manual_type) : mono_type =
  ...
  let aux_labeled_list = decode_manual_record_type pre in
  ...
  let rec aux (rng, mtymain) =
    let tymain =
      ...
      | MFuncType ((mtydoms, mndlabmtys, mrow), mtycod) ->
          ...
          FuncType
            ( {
                ordered = List.map aux mtydoms;
                mandatory = mndlabmap;
                optional = optrow;
              },
              aux mtycod )
      | MProductType mtys -> ProductType (TupleList.map aux mtys)
      | MRecordType mrow ->
      ...
      | MTypeVar typaram -> (
          ...
          | None -> raise_error (UnboundTypeParameter (rng, typaram))
          | Some mbbid -> TypeVar (MustBeBound mbbid))
      | MModProjType (utmod1, tyident2, mtyargs) -> (
          let rng2, tynm2 = tyident2 in
          let absmodsig1, _ =
            typecheck_module ~address:Address.root tyenv utmod1
          in
          ...
          | _, ConcStructure sigr -> (
              ...
              | None -> raise_error (UndefinedTypeName (rng2, tynm2))
              | Some tentry2 -> (
                  let tyargs = mtyargs |> List.map aux in
                  let len_actual = List.length tyargs in
                  ...
                  | Some ((_, tymain) as ty) ->
                      if opaque_occurs_in_mono_type quant1 ty then
                        ...
                        raise_error
                          (OpaqueIDExtrudesScopeViaType (rng, tentry2))
                      else tymain
                  | None ->
      ...
      | MPackType utsig ->
          let absmodsig =
            typecheck_signature ~address:Address.root tyenv utsig
          in
          PackType absmodsig
    in
    (rng, tymain)
    ...
    | MRow (optlabmtys, rowvar_opt) ->
        let row_last =
          ...
          | Some (rng, rowparam) -> (
              ...
              | None -> raise_error (UnboundRowParameter (rng, rowparam))
              | Some (mbbrid, _) -> RowVar (MustBeBoundRow mbbrid))
        in
        optlabmtys
        |> List.fold_left
             (fun row_acc (rlabel, mty) ->
               let ty = aux mty in
...
and decode_manual_record_type (pre : pre) (labmtys : labeled_manual_type list) :
    mono_type LabelAssoc.t =
  let aux = decode_manual_type pre in
  labmtys
  |> List.fold_left
       (fun labmap (rlabel, mty) ->
...
and add_local_row_parameter
    (rowvars : (row_variable_name ranged * label ranged list) list) (pre : pre)
    : pre =
  rowvars
  |> List.fold_left
       (fun pre ((rng, rowvarnm), mkind) ->
         let rowparams = pre.local_row_parameters in
             ...
             |> List.fold_left
                  (fun labset rlabel ->
                    ...
                    if labset |> LabelSet.mem label then
                      raise_error (DuplicatedLabel (rnglabel, label))
                    else labset |> LabelSet.add label)
                  LabelSet.empty
           in
           KindStore.register_bound_row
             (MustBeBoundRowID.to_bound mbbrid)
             labset;
           let rowparams =
             rowparams |> RowParameterMap.add rowvarnm (mbbrid, labset)
           in
           { pre with local_row_parameters = rowparams })
       pre

and decode_type_annotation_or_fresh (pre : pre) (((rng, x), tyannot) : binder) :
    mono_type =
  match tyannot with
  | None -> fresh_type_variable ~name:x pre.level rng
  | Some mty -> decode_manual_type pre mty

and decode_parameter (pre : pre) (binder : binder) :
    mono_type * pattern * binding_map =
...
and add_binding_map_to_type_environment (bindmap : binding_map)
    (tyenv : Typeenv.t) : Typeenv.t =
  BindingMap.fold
    (fun x (ty, lname, _) tyenv ->
      tyenv
      |> Typeenv.add_value x (TypeConv.lift ty) (OutputIdentifier.Local lname))
    bindmap tyenv

and add_ordered_parameters_to_type_environment (pre : pre)
    (binders : binder list) : Typeenv.t * mono_type list * pattern list =
  let tyenv, ipatacc, tydomacc =
    List.fold_left
      (fun (tyenv, ipatacc, ptydomacc) binder ->
      ...
      (pre.tyenv, Alist.empty, Alist.empty)
      binders
  in
...
and add_labeled_optional_parameters_to_type_environment (pre : pre)
    (optbinders : (labeled_binder * untyped_ast option) list) :
    Typeenv.t * mono_row * (pattern * ast option) LabelAssoc.t =
  optbinders
  |> List.fold_left
       (fun (tyenv, optrow, optipatmap) ((rlabel, binder), utdefault) ->
         let rnglabel, label = rlabel in
         if optipatmap |> LabelAssoc.mem label then
           ...
           let ty_outer, default =
             ...
             | None ->
                 let ty_outer =
                   fresh_type_variable pre.level (Range.dummy "optional")
                 in
                 unify ty_inner
                   (Primitives.option_type (Range.dummy "option") ty_outer);
                 (ty_outer, None)
             | Some utast ->
           ...
           let tyenv = tyenv |> add_binding_map_to_type_environment bindmap in
           let optipatmap =
             optipatmap |> LabelAssoc.add label (ipat, default)
           in
           (tyenv, optrow, optipatmap))
       (pre.tyenv, RowEmpty, LabelAssoc.empty)

and add_labeled_mandatory_parameters_to_type_environment (pre : pre)
    (mndbinders : labeled_binder list) :
    Typeenv.t * mono_type LabelAssoc.t * pattern LabelAssoc.t =
  mndbinders
  |> List.fold_left
       (fun (tyenv, labmap, optipatmap) (rlabel, binder) ->
         let rnglabel, label = rlabel in
         if labmap |> LabelAssoc.mem label then
           raise_error (DuplicatedLabel (rnglabel, label))
         else
...
and add_parameters_to_type_environment (pre : pre)
    ((ordbinders, mndbinders, optbinders) : untyped_parameters) =
  let tyenv, tydoms, ordnames =
    add_ordered_parameters_to_type_environment pre ordbinders
  in
  let tyenv, mndlabmap, mndnamemap =
    add_labeled_mandatory_parameters_to_type_environment { pre with tyenv }
      mndbinders
  in
  let tyenv, optrow, optnamemap =
    add_labeled_optional_parameters_to_type_environment { pre with tyenv }
      optbinders
  in
...
and typecheck (pre : pre) ((rng, utastmain) : untyped_ast) : mono_type * ast =
  ...
  | Var (modidents1, (rng2, x2)) -> (
      ...
      | [] -> (
          ...
          | None -> raise_error (UnboundVariable (rng2, x2))
          | Some ((_, ptymain), name) ->
...
*)
  ...
  | Apply (utastfun, utargs) -> (
      ...
      | _, FuncType (domain_expected, tyret) ->
          ...
          let iargs =
            typecheck_arguments_against_domain pre rng utargs domain_expected
          in
          let tyret =
  ...
  | Freeze (rngapp, frozenfun, utastargs, restrngs) ->
      let ptyfun, gname =
        ...
        | FrozenFun (rng0, x) -> (
            ...
            | None -> raise_error (UnboundVariable (rng0, x))
            | Some ((_, ptymain), name) -> (
                match name with
      ...
      in
      ( Primitives.frozen_type rng ~rest:tyrest ~receive:tyrecv ~return:tyret,
        IFreeze (gname, eargs) )
  | FreezeUpdate (utast0, utastargs, restrngs) ->
      ...
      in
      let tyrecv =
        fresh_type_variable ~name:"FreezeUpdate, recv" pre.level
          (Range.dummy "FreezeUpdate, recv")
      in
      let tyret =
        fresh_type_variable ~name:"FreezeUpdate, ret" pre.level
          (Range.dummy "FreezeUpdate, ret")
      in
      let ty_expected =
        let tyrest_expected =
          ...
          | ty :: tys -> (dr, ProductType (TupleList.make ty tys))
        in
        Primitives.frozen_type
          (Range.dummy "FreezeUpdate")
          ~rest:tyrest_expected ~receive:tyrecv ~return:tyret
      in
  ...
  | If (utast0, utast1, utast2) ->
      ...
      unify ty1 ty2;
      let ibranches =
        [ IBranch (IPBool true, e1); IBranch (IPBool false, e2) ]
      in
  ...
  | LetIn (NonRec letbind, utast2) ->
      ...
      let tyenv =
        ...
        pre.tyenv |> Typeenv.add_value x pty (OutputIdentifier.Local lname)
      in
  ...
  | LetIn (Rec letbinds, utast2) ->
      ...
      let ty2, e2 =
        let tyenv =
          binds
          |> List.fold_left
               (fun tyenv (x, pty, lname_outer, _, _) ->
                 tyenv
                 |> Typeenv.add_value x pty (OutputIdentifier.Local lname_outer))
               pre.tyenv
        in
  ...
  | Case (utast0, branches) ->
      ...
      let ibrs =
        branches
        |> List.map (typecheck_pure_case_branch pre ~pattern:ty0 ~return:tyret)
      in
      (tyret, ICase (e0, ibrs))
  | LetPatIn (utpat, utast1, utast2) ->
      let tyenv, ipat, bindmap, e1 =
        typecheck_let_pattern pre rng utpat utast1
      in
      let ty2, e2 = typecheck { pre with tyenv } utast2 in
      check_binding_map_properly_used tyenv bindmap;
      (ty2, iletpatin ipat e1 e2)
  | Constructor (modidents, ctornm, utastargs) -> (
      let tyid, ctorid, tyargs, tys_expected =
        typecheck_constructor pre rng modidents ctornm
      in
      ...
      | exception Invalid_argument _ ->
          ...
          raise_error
            (InvalidNumberOfConstructorArguments
               (rng, ctornm, len_expected, len_actual))
      | zipped ->
          let eacc =
            zipped
            |> List.fold_left
                 (fun eacc (utast, ty_expected) ->
                   let ty, e = typecheck pre utast in
  ...
  | BinaryByList nrs ->
      let ns =
        nrs
        |> List.map (fun (rngn, n) ->
               if 0 <= n && n <= 255 then n else raise_error (InvalidByte rngn))
      in
  ...
  | RecordAccess (utast1, ((_, label) as rlabel)) ->
      ...
      unify ty1
        ( Range.dummy "RecordAccess",
          RecordType (RowCons (rlabel, ty_ret, row_rest)) );
  ...
  | RecordUpdate (utast1, ((_, label) as rlabel), utast2) ->
      ...
      let row_rest = fresh_row_variable pre.level (LabelSet.singleton label) in
      unify ty1
        ( Range.dummy "RecordUpdate",
          RecordType (RowCons (rlabel, ty2, row_rest)) );
  ...
  | Pack (modidentchain1, utsig2) ->
      ...
      let absmodsig2 =
        typecheck_signature ~address:Address.root pre.tyenv utsig2
      in
      let absmodsig =
        coerce_signature ~cause:rng ~address:Address.root modsig1 absmodsig2
      in
...
and typecheck_let_pattern (pre : pre) (rng : Range.t) (utpat : untyped_pattern)
    (utast1 : untyped_ast) =
...
and typecheck_computation (pre : pre) (utcomp : untyped_computation_ast) :
    (mono_effect * mono_type) * ast =
  ...
  | CompReceive (branches, after_opt) ->
      ...
      let ibrs =
        branches |> List.map (typecheck_receive_branch pre effexp tyret)
      in
      let iafter_opt =
        after_opt
        |> Option.map (fun (utast1, utcomp2) ->
               let ty1, e1 = typecheck pre utast1 in
  ...
  | CompCase (utast0, branches) ->
      ...
      let ibrs =
        branches
        |> List.map
             (typecheck_effectful_case_branch pre ~pattern:ty0
                ~return:(eff, tyret))
      in
...
and get_structure_signature (tyenv : Typeenv.t) (modident : module_name ranged)
    (projs : module_name ranged list) : SigRecord.t =
...
and typecheck_arguments (pre : pre) (rng : Range.t)
    ((utastargs, mndutastargs, optutastargs) : untyped_arguments) =
  let tyeargs = List.map (typecheck pre) utastargs in
  let tyargs = List.map fst tyeargs in
  let eargs = List.map snd tyeargs in
  ...
  let mndlabmap, mndargmap =
    mndutastargs
    |> List.fold_left
         (fun (mndlabmap, mndargmap) (rlabel, utast) ->
           let rnglabel, label = rlabel in
  ...
  let optrow, optargmap =
    ...
    let optrow, optlabset, optargmap =
      optutastargs
      |> List.fold_left
           (fun (optrow, optlabset, optargmap) (rlabel, utast) ->
             let rnglabel, label = rlabel in
...
and typecheck_arguments_against_domain (pre : pre) (rng : Range.t)
    ((utastargs, mndutastargs, optutastargs) : untyped_arguments)
    (domain_expected : mono_domain_type) =
  let {
    ordered = tys_expected;
    mandatory = mndlabmap_expected;
    optional = optrow_expected;
  } =
    domain_expected
  in
  let eargs =
    ...
    if numord_got = numord_expected then
      List.fold_left2
        (fun eargacc utastarg ty_expected ->
          let ty_got, e = typecheck pre utastarg in
          unify ty_got ty_expected;
          Alist.extend eargacc e)
        Alist.empty utastargs tys_expected
      |> Alist.to_list
    else
      raise_error
      @@ BadArityOfOrderedArguments
           { range = rng; got = numord_got; expected = numord_expected }
  in
  let mndargmap =
    let mndlabmap_rest, mndargmap =
      mndutastargs
      |> List.fold_left
           (fun (mndlabmap_rest, mndargmap) (rlabel, utast) ->
             let rnglabel, label = rlabel in
             if mndargmap |> LabelAssoc.mem label then
               ...
               | None ->
                   raise_error
                   @@ UnexpectedMandatoryLabel { range = rnglabel; label }
               | Some ty_expected ->
                   let ty_got, e = typecheck pre utast in
                   unify ty_got ty_expected;
                   let mndlabmap_rest =
                     mndlabmap_rest |> LabelAssoc.remove label
                   in
                   let mndargmap = mndargmap |> LabelAssoc.add label e in
    ...
    | [] -> mndargmap
    | (label, ty) :: _ ->
        raise_error @@ MissingMandatoryLabel { range = rng; label; typ = ty }
  in
  let optargmap =
    let (NormalizedRow (labmap_known, rowvar_opt)) =
      TypeConv.normalize_mono_row optrow_expected
    in
    let all_labset, unknown_labels, optargmap =
      optutastargs
      |> List.fold_left
           (fun (all_labset, unknown_labels, optargmap) (rlabel, utast) ->
             ...
             if optargmap |> LabelAssoc.mem label then
               ...
               | None ->
                   ( all_labset,
                     unknown_labels |> LabelAssoc.add label (rng_label, ty_got),
                     optargmap )
               | Some ty_expected ->
                   unify ty_got ty_expected;
    ...
    | (label, _) :: _ -> (
        ...
        | Some (UpdatableRow ({ contents = FreeRow frid } as mrvu)) ->
            let row_unknown =
              let row_init =
                let frid0 =
                  FreeRowID.fresh ~message:"typecheck_arguments_against_domain"
                    pre.level
                in
...
*)
        ...
        | _ -> raise_error @@ UnexpectedOptionalLabel { range = rng; label })
    | _ -> ());
    optargmap
  in
  (eargs, mndargmap, optargmap)

and typecheck_constructor (pre : pre) (rng : Range.t)
    (modidents : module_name ranged list) (ctornm : constructor_name) =
  match modidents with
  | [] -> (
      match pre.tyenv |> Typeenv.find_constructor ctornm with
      | None -> raise_error (UndefinedConstructor (rng, ctornm))
      | Some centry ->
          let tyid = centry.belongs in
          let ctorid = centry.constructor_id in
          let bids = centry.type_variables in
          let ptys = centry.parameter_types in
          let tyargs, tys_expected =
            TypeConv.instantiate_type_arguments pre.level bids ptys
          in
...
and typecheck_pure_case_branch (pre : pre) ~pattern:typatexp ~return:tyret
    (CaseBranch (pat, utast1)) =
...
and typecheck_effectful_case_branch (pre : pre) ~pattern:typatexp
    ~return:(eff, tyret) (CompCaseBranch (pat, utcomp1)) =
  let typat, ipat, bindmap = typecheck_pattern pre pat in
  let tyenv = pre.tyenv |> add_binding_map_to_type_environment bindmap in
...
and typecheck_receive_branch (pre : pre) (effexp : mono_effect)
    (tyret : mono_type) (ReceiveBranch (pat, utcomp1)) =
  let typat, ipat, bindmap = typecheck_pattern pre pat in
  let tyenv = pre.tyenv |> add_binding_map_to_type_environment bindmap in
  let (eff1, ty1), e1 = typecheck_computation { pre with tyenv } utcomp1 in
  check_binding_map_properly_used tyenv bindmap;
...
and check_binding_map_properly_used (tyenv : Typeenv.t) (bindmap : binding_map)
    : unit =
  BindingMap.iter
    (fun x (_, _, rng) -> check_properly_used tyenv (rng, x))
    bindmap
...
and typecheck_pattern (pre : pre) ((rng, patmain) : untyped_pattern) :
  ...
  | PChar uchar -> immediate (BaseType CharType) (IPChar uchar)
  | PVar x ->
  ...
  | PTuple pats ->
      ...
      let bindmaps =
        triples |> TupleList.map (fun (_, _, bindmap) -> bindmap)
      in
      let bindmap =
  ...
  | PConstructor (modidents, ctornm, pats) -> (
      ...
      try
        let ipatacc, bindmap =
          List.fold_left2
            (fun (ipatacc, bindmapacc) ty_expected pat ->
              let ty, ipat, bindmap = typecheck_pattern pre pat in
              unify ty ty_expected;
              ( Alist.extend ipatacc ipat,
                binding_map_union rng bindmapacc bindmap ))
            (Alist.empty, BindingMap.empty)
            tys_expected pats
        in
        let ty = (rng, TypeApp (tyid, tyargs)) in
 ...
 fun namef preL letbind ->
  ...
  let ty1, e0, ibinders =
    ...
    let preS =
      ...
      { preL with level = preL.level + 1 }
      |> add_local_row_parameter letbind.vb_forall_row
    in

    (* Second, add local value parameters at level `levS`. *)
    let tyenv, domain, ibinders =
      add_parameters_to_type_environment preS (ordparams, mndparams, optparams)
    in
...
and typecheck_letrec_mutual :
      'n.
      (untyped_let_binding -> 'n * 'n) ->
      ('n -> name) ->
      pre ->
      untyped_let_binding list ->
      (identifier * poly_type * 'n * 'n * ast) list =
 fun namesf proj preL letbinds ->
  ...
  let tupleacc, tyenv =
    letbinds
    |> List.fold_left
         (fun (tupleacc, tyenv) letbind ->
           let rngv, x = letbind.vb_identifier in
           let name_inner, name_outer = namesf letbind in
           let preS, ptyopt =
             make_rec_initial_type_from_annotation preL letbind
           in
           let tyenv, morph =
             match ptyopt with
             | Some pty ->
                 let tyenv =
                   tyenv |> Typeenv.add_value x pty (proj name_inner)
                 in
                 (tyenv, PolyRec pty)
             | None ->
                 ...
                 let tyenv =
                   tyenv
                   |> Typeenv.add_value x (TypeConv.lift tyf) (proj name_inner)
                 in
                 (tyenv, MonoRec tyf)
           in
           ( Alist.extend tupleacc (letbind, name_inner, name_outer, morph, preS),
             tyenv ))
         (Alist.empty, preL.tyenv)
  in

  let bindacc =
    tupleacc |> Alist.to_list
    |> List.fold_left
         (fun bindacc (letbind, name_inner, name_outer, morph, preS) ->
           let preS = { preS with tyenv } in
...
and typecheck_letrec_single (preS : pre) (letbind : untyped_let_binding)
    (morph : rec_morph) : poly_type * ast =
  let rngv, x = letbind.vb_identifier in
  let ordparams = letbind.vb_parameters in
  let mndparams = letbind.vb_mandatories in
  let optparams = letbind.vb_optionals in
  ...
  (match morph with
  | MonoRec tyf -> unify ty1 tyf
  | PolyRec ptyannot ->
      if subtype_poly_type ptyf ptyannot then ()
      else raise_error (PolymorphicContradiction (rngv, x, ptyf, ptyannot)));
  (ptyf, e1)
         ...
         | ConstructorBranch (attrs, (rng, ctornm), mtyargs) ->
             ...
             let ctorid =
               ...
               | Some (rng_atom, target_atom) -> (
                   ...
                   | Some ctorid -> ctorid
                   | None ->
                       raise_error (InvalidIdentifier (rng_atom, target_atom)))
             in
...
*)
and subtype_poly_type_impl (internbid : type_intern) (internbrid : row_intern)
    (pty1 : poly_type) (pty2 : poly_type) : bool =
...
*)
    ...
    | FuncType (pdomain1, ptycod1), FuncType (pdomain2, ptycod2) ->
        ...
        bdom && bcod
    | PidType pidty1, PidType pidty2 -> aux_pid pidty1 pidty2
    ...
    | PackType absmodsig1, PackType absmodsig2 -> (
        try
          ...
          true
        with _ -> false)
    | TypeVar (Bound bid1), _ -> internbid bid1 pty2
  ...
  and aux_domain domain1 domain2 =
    let { ordered = ptydoms1; mandatory = mndlabmap1; optional = poptrow1 } =
      domain1
    in
    let { ordered = ptydoms2; mandatory = mndlabmap2; optional = poptrow2 } =
      domain2
    in
    let b1 = aux_list ptydoms1 ptydoms2 in
    let bmnd =
      subtype_label_assoc_with_equal_domain internbid internbrid mndlabmap1
        mndlabmap2
    in
    let bopt =
    ...
    in
    b1 && bmnd && bopt
  and aux_pid (Pid pty1) (Pid pty2) = aux pty1 pty2
  and aux_effect (Effect pty1) (Effect pty2) = aux pty1 pty2 in
  aux pty1 pty2
...
and subtype_label_assoc_inclusive (internbid : type_intern)
    (internbrid : row_intern) (plabmap1 : poly_type LabelAssoc.t)
    (plabmap2 : poly_type LabelAssoc.t) : poly_type LabelAssoc.t option =
  let merged =
    LabelAssoc.merge
      (fun label pty1_opt pty2_opt ->
        match (pty1_opt, pty2_opt) with
        | Some pty1, Some pty2 ->
            Some (Ok (subtype_poly_type_impl internbid internbrid pty1 pty2))
  ...
  if
    merged
    |> LabelAssoc.for_all (fun _label res -> Result.value ~default:true res)
  then
    let plabmap_diff =
      merged
      |> LabelAssoc.filter_map (fun _label res ->
             match res with Ok _ -> None | Error pty2 -> Some pty2)
    in
    Some plabmap_diff
  else None

and subtype_label_assoc_with_equal_domain (internbid : type_intern)
    (internbrid : row_intern) (plabmap1 : poly_type LabelAssoc.t)
    (plabmap2 : poly_type LabelAssoc.t) : bool =
  LabelAssoc.merge
    (fun label pty1_opt pty2_opt ->
      match (pty1_opt, pty2_opt) with
      | Some pty1, Some pty2 ->
          Some (subtype_poly_type_impl internbid internbrid pty1 pty2)
      | _ -> Some false)
    plabmap1 plabmap2
  |> LabelAssoc.for_all (fun _label b -> b)

and subtype_row_with_equal_domain (internbid : type_intern)
    (internbrid : row_intern) (prow1 : poly_row) (prow2 : poly_row) : bool =
  (*
  ...
  Format.printf "!!! - %a\n" (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf ", ") Format.pp_print_string) (List.concat [sbt1; sbr1; sbt2; sbr2]);
*)
  let (NormalizedRow (plabmap1, rowvar1_opt)) =
    TypeConv.normalize_poly_row prow1
  in
  let (NormalizedRow (plabmap2, rowvar2_opt)) =
    TypeConv.normalize_poly_row prow2
  in

  match (rowvar1_opt, rowvar2_opt) with
  | None, None ->
      subtype_label_assoc_with_equal_domain internbid internbrid plabmap1
        plabmap2
  | Some (MonoRow _), _ | _, Some (MonoRow _) -> assert false
  | None, Some (BoundRow _brid2) -> false
  | Some (BoundRow brid1), _ -> (
      let opt =
        subtype_label_assoc_inclusive internbid internbrid plabmap1 plabmap2
      in
      match opt with
      | None -> false
      | Some plabmap_diff ->
          internbrid brid1 (NormalizedRow (plabmap_diff, rowvar2_opt)))
...
and subtype_poly_type (pty1 : poly_type) (pty2 : poly_type) : bool =
  ...
  let internbrid (brid1 : BoundRowID.t) (nomrow2 : normalized_poly_row) : bool =
    ...
    | None ->
        ...
        true
    | Some nomrow -> normalized_poly_row_equal nomrow nomrow2
  in
...
and normalized_poly_row_equal (nomrow1 : normalized_poly_row)
    (nomrow2 : normalized_poly_row) : bool =
  ...
  let bmap =
    LabelAssoc.merge
      (fun _ ptyopt1 ptyopt2 ->
        ...
        | _ -> Some false)
      plabmap1 plabmap2
    |> LabelAssoc.for_all (fun _ b -> b)
  in
  if bmap then
    match (rowvar1_opt, rowvar2_opt) with
    | None, None -> true
    | Some (BoundRow brid1), Some (BoundRow brid2) ->
...
and poly_type_equal (pty1 : poly_type) (pty2 : poly_type) : bool =
  let rec aux (pty1 : poly_type) (pty2 : poly_type) : bool =
    ...
    | PidType ppidty1, PidType ppidty2 -> aux_pid_type ppidty1 ppidty2
    | ProductType ptys1, ProductType ptys2 ->
        aux_list (ptys1 |> TupleList.to_list) (ptys2 |> TupleList.to_list)
    | RecordType prow1, RecordType prow2 -> poly_row_equal prow1 prow2
    | PackType absmodsig1, PackType absmodsig2 -> (
        try
          subtype_abstract_with_abstract
    ...
    | TypeVar (Bound bid1), TypeVar (Bound bid2) -> BoundID.equal bid1 bid2
    | TypeVar (Mono _), _ | _, TypeVar (Mono _) -> assert false
    | _ -> false
  and aux_list tys1 tys2 =
    try List.fold_left2 (fun b ty1 ty2 -> b && aux ty1 ty2) true tys1 tys2
    with Invalid_argument _ -> false
  and aux_domain pdomain1 pdomain2 =
    let { ordered = pty1doms; mandatory = pmndlabmap1; optional = poptrow1 } =
      pdomain1
    in
    let { ordered = pty2doms; mandatory = pmndlabmap2; optional = poptrow2 } =
      pdomain2
    in
    aux_list pty1doms pty2doms
    && poly_label_assoc_equal pmndlabmap1 pmndlabmap2
    && poly_row_equal poptrow1 poptrow2
  and aux_effect (Effect pty1) (Effect pty2) = aux pty1 pty2
  and aux_pid_type (Pid pty1) (Pid pty2) = aux pty1 pty2 in
...
and subtype_type_scheme (tyscheme1 : type_scheme) (tyscheme2 : type_scheme) :
    bool * BoundID.t BoundIDMap.t =
  let bids1, pty_body1 = tyscheme1 in
  let bids2, pty_body2 = tyscheme2 in
  match List.combine bids1 bids2 with
  | exception Invalid_argument _ -> (false, BoundIDMap.empty)
  | zipped ->
      let bidmap =
        zipped
        |> List.fold_left
             (fun bidmap (bid1, bid2) -> bidmap |> BoundIDMap.add bid1 bid2)
             BoundIDMap.empty
      in
...
and lookup_type_entry (tynm : type_name) (tentry1 : type_entry)
    (tentry2 : type_entry) : substitution option =
...
and lookup_record (rng : Range.t) (modsig1 : module_signature)
    (modsig2 : module_signature) : substitution =
  let take_left _tyid to1 _to2 = Some to1 in
  match (modsig1, modsig2) with
  | (_, ConcStructure sigr1), (_, ConcStructure sigr2) ->
      sigr2
      |> SigRecord.fold
           ~v:(fun _x2 _ventry2 subst -> subst)
           ~c:(fun ctornm2 _centry2 subst -> subst)
           ~f:(fun tynm2 _pty2 subst -> subst)
           ~t:(fun tynm2 tentry2 subst ->
             ...
             | Some tentry1 -> (
                 ...
                 | None ->
                     raise_error
                       (NotASubtypeTypeDefinition (rng, tynm2, tentry1, tentry2))
           ...
           ~s:(fun _ _ subst -> subst)
           SubstMap.empty
  | _ -> SubstMap.empty

and subtype_abstract_with_abstract ~(cause : Range.t) ~(address : Address.t)
    (absmodsig1 : module_signature abstracted)
    (absmodsig2 : module_signature abstracted) : unit =
...
and subtype_concrete_with_concrete ~(cause : Range.t) ~(address : Address.t)
    (modsig1 : module_signature) (modsig2 : module_signature) : unit =
  match (modsig1, modsig2) with
  | (_, ConcFunctor sigftor1), (_, ConcFunctor sigftor2) ->
      let quant1, Domain (isig1, sigr1), absmodsigcod1 =
        (sigftor1.opaques, sigftor1.domain, sigftor1.codomain)
      in
      let quant2, Domain (isig2, sigr2), absmodsigcod2 =
        (sigftor2.opaques, sigftor2.domain, sigftor2.codomain)
      in
      let subst =
        let modsigdom1 = (isig1, ConcStructure sigr1) in
        let modsigdom2 = (isig2, ConcStructure sigr2) in
        subtype_concrete_with_abstract ~cause ~address modsigdom2
          (quant1, modsigdom1)
      in
  ...
  | (_, ConcStructure sigr1), (_, ConcStructure sigr2) ->
      ...
      |> SigRecord.fold
           ~v:(fun x2 ventry2 () ->
             ...
             match sigr1 |> SigRecord.find_value x2 with
             | None -> raise_error (MissingRequiredValName (cause, x2, pty2))
             | Some ventry1 ->
                 let pty1 = ventry1.val_type in
                 if subtype_poly_type pty1 pty2 then ()
                 else
                   raise_error
                     (PolymorphicContradiction (cause, x2, pty1, pty2)))
           ~c:(fun ctornm2 centry2 () ->
             match sigr1 |> SigRecord.find_constructor ctornm2 with
             | None ->
                 raise_error
                   (MissingRequiredConstructorName (cause, ctornm2, centry2))
             | Some centry1 ->
                 let tyscheme1 =
                   make_type_scheme_from_constructor_entry centry1
                 in
                 let tyscheme2 =
                   make_type_scheme_from_constructor_entry centry2
                 in
                 let b, _ = subtype_type_scheme tyscheme1 tyscheme2 in
                 if b then ()
                 else
                   raise_error
                     (NotASubtypeConstructorDefinition
                        (cause, ctornm2, centry1, centry2)))
           ~f:(fun tynm2 pty2 () ->
             ...
             | None -> (
                 ...
                 | Some tentry2 ->
                     raise_error
                       (MissingRequiredTypeName (cause, tynm2, tentry2)))
             | Some pty1 -> (
                 if subtype_poly_type pty1 pty2 then ()
                 else
                   match
                     ( sigr1 |> SigRecord.find_type tynm2,
                       sigr2 |> SigRecord.find_type tynm2 )
                   with
                   | Some tentry1, Some tentry2 ->
                       raise_error
                         (NotASubtypeTypeDefinition
                            (cause, tynm2, tentry1, tentry2))
                   | _ -> assert false))
           ~t:(fun tynm2 tentry2 () ->
             match sigr1 |> SigRecord.find_type tynm2 with
             | None ->
             ...
             | Some tentry1 ->
                 ...
                 if b1 && b2 && b0 then ()
                   ...
                   raise_error
                     (NotASubtypeTypeDefinition (cause, tynm2, tentry1, tentry2)))
           ~m:(fun modnm2 mentry2 () ->
             let modsig2 = mentry2.mod_signature in
             match sigr1 |> SigRecord.find_module modnm2 with
             | None ->
                 raise_error
                   (MissingRequiredModuleName (cause, modnm2, modsig2))
             | Some mentry1 ->
                 let modsig1 = mentry1.mod_signature in
           ...
           ~s:(fun signm2 sentry2 () ->
             ...
             | None ->
                 raise_error
                   (MissingRequiredSignatureName (cause, signm2, absmodsig2))
             | Some sentry1 ->
                 let absmodsig1 = sentry1.sig_signature in
                 subtype_abstract_with_abstract ~cause ~address absmodsig1
                   absmodsig2;
                 subtype_abstract_with_abstract ~cause ~address absmodsig2
                   absmodsig1;
                 ())
           ()
  | _ -> raise_error (NotASubtype (cause, modsig1, modsig2))

and subtype_concrete_with_abstract ~(cause : Range.t) ~(address : Address.t)
    (modsig1 : module_signature) (absmodsig2 : module_signature abstracted) :
    substitution =
...
and subtype_signature ~(cause : Range.t) ~(address : Address.t)
    (modsig1 : module_signature) (absmodsig2 : module_signature abstracted) =
  subtype_concrete_with_abstract ~cause ~address modsig1 absmodsig2

and substitute_signature_source (subst : substitution) (isig : signature_source)
    : signature_source =
  isig
(* TODO *)

and substitute_concrete ~(cause : Range.t) (subst : substitution)
    (modsig : module_signature) : module_signature =
  match modsig with
  | isig, ConcFunctor sigftor ->
      let quant, Domain (isigdom, sigr), absmodsigcod =
        (sigftor.opaques, sigftor.domain, sigftor.codomain)
      in
...
and copy_closure (modsig1 : module_signature) (modsig2 : module_signature) :
    module_signature =
  match (modsig1, modsig2) with
...
and copy_closure_in_structure (sigr1 : SigRecord.t) (sigr2 : SigRecord.t) :
    SigRecord.t =
  sigr2
  |> SigRecord.map
       ~v:(fun x ventry2 ->
       ...
       ~m:(fun modnm mentry2 ->
         ...
         | Some mentry1 ->
             let modsig2 =
               copy_closure mentry1.mod_signature mentry2.mod_signature
             in
             {
               ...
               mod_doc =
                 mentry2.mod_doc
                 (* Should use `mentry2`, not `mentry1` for doc comments. *);
             })
       ~s:(fun signm sentry2 ->
...
and update_subsignature (modnms : module_name list)
    (updater : module_signature -> module_signature) (modsig : module_signature)
    : module_signature =
  match modnms with
  | [] -> updater modsig
  | modnm0 :: modnms -> (
      match modsig with
      | _, ConcFunctor _ -> modsig
      | isig, ConcStructure sigr ->
          let sigr =
            sigr
            |> SigRecord.map
                 ...
                 ~m:(fun modnm mentry ->
                   if String.equal modnm modnm0 then
                     let modsig =
                       mentry.mod_signature
                       |> update_subsignature modnms updater
                     in
...
and substitute_type_entity ~(cause : Range.t) (bids_source : BoundID.t list)
    (subst : substitution) (tyentity : type_entity) : type_entity =
  match tyentity with
  | Opaque tyid_from -> (
      match subst |> SubstMap.find_opt tyid_from with
          ...
          | Variant ctormap_target ->
              let bidmap =
                ...
                | zipped ->
                    zipped
                    |> List.fold_left
                         (fun bidmap (bid_from, bid_to) ->
                           let pty_to =
                             ( Range.dummy "substitute_type_entity",
                               TypeVar (Bound bid_to) )
                           in
              ...
              let ctormap =
                ctormap_target
                |> ConstructorMap.map (fun (ctorid, ptys) ->
                       ( ctorid,
                         ptys
                         |> List.map (fun pty ->
...
and substitute_structure ~(cause : Range.t) (subst : substitution)
    (sigr : SigRecord.t) : SigRecord.t =
  sigr
  |> SigRecord.map
       ~v:(fun _x ventry ->
         {
           ventry with
           val_type = ventry.val_type |> substitute_poly_type ~cause subst;
         })
       ~c:(fun _ctornm centry ->
         {
           ...
           parameter_types =
             centry.parameter_types
             |> List.map (substitute_poly_type ~cause subst);
         })
       ~f:(fun _tynm pty -> pty |> substitute_poly_type ~cause subst)
       ~t:(fun _tynm tentry ->
       ...
       ~m:(fun _ mentry ->
         {
           mentry with
           mod_signature =
             mentry.mod_signature |> substitute_concrete ~cause subst;
         })
       ~s:(fun _ sentry ->
         let absmodsig =
           sentry.sig_signature |> substitute_abstract ~cause subst
         in
         { sentry with sig_signature = absmodsig })

and substitute_abstract ~(cause : Range.t) (subst : substitution)
    (absmodsig : module_signature abstracted) : module_signature abstracted =
...
and substitute_poly_type ~(cause : Range.t) (subst : substitution)
    (pty : poly_type) : poly_type =
  let rec aux (rng, ptymain) =
    let ptymain =
      ...
      | FuncType (pdomain, ptycod) -> FuncType (aux_domain pdomain, aux ptycod)
      | RecordType prow -> RecordType (aux_row prow)
      | TypeApp (tyid_from, ptyargs) -> (
          match subst |> SubstMap.find_opt tyid_from with
          | None -> TypeApp (tyid_from, ptyargs |> List.map aux)
          | Some (bids, pty_body, _) -> (
              let tyscheme = (bids, pty_body) in
              match
                TypeConv.apply_type_scheme_poly tyscheme
                  (ptyargs |> List.map aux)
              with
  ...
  and aux_domain pdomain =
    let { ordered = ptydoms; mandatory = pmndlabmap; optional = poptrow } =
      pdomain
    in
    {
  ...
  and aux_pid = function Pid pty -> Pid (aux pty)
  and aux_effect = function Effect pty -> Effect (aux pty)
...
and typecheck_declaration ~(address : Address.t) (tyenv : Typeenv.t)
    (utdecl : untyped_declaration) : SigRecord.t abstracted =
  ...
  | DeclVal ((_, x), typarams, rowparams, mty, attrs) ->
      ...
      let ventry =
        { val_type = pty; val_global = gname; val_doc = declattr.doc }
      in
  ...
  | DeclTypeOpaque (tyident, kdannot, attrs) ->
      ...
      let tentry =
        let bids, pty_body =
          TypeConv.make_opaque_type_scheme_from_base_kinds bkds oid
        in
        {
  ...
  | DeclModule (modident, utsig, attrs) ->
      ...
      let absmodsig =
        typecheck_signature
          ~address:(address |> Address.append_member m)
          tyenv utsig
      in
      let quant, modsig = absmodsig in
      let sname = get_space_name rngm m in
      let mentry =
        { mod_signature = modsig; mod_name = sname; mod_doc = declattr.doc }
      in
  ...
  | DeclInclude utsig -> (
      ...
      | isig, ConcStructure sigr -> (quant, sigr))

and typecheck_declaration_list ~(address : Address.t) (tyenv : Typeenv.t)
    (utdecls : untyped_declaration list) : SigRecord.t abstracted =
  let quantacc, sigracc, _ =
    utdecls
    |> List.fold_left
         (fun (quantacc, sigracc, tyenv) ((rng, _) as utdecl) ->
...
and copy_abstract_signature ~(cause : Range.t) ~(address_to : Address.t)
    (absmodsig_from : module_signature abstracted) : module_signature abstracted
    =
  let quant_from, modsig_from = absmodsig_from in
  let quant_to, subst =
    OpaqueIDMap.fold
      (fun oid_from pkd (quant_to, subst) ->
        let oid_to =
        ...
        let bids, pty_body =
          TypeConv.make_opaque_type_scheme_from_base_kinds pbkds oid_to
        in
        let subst =
          subst |> SubstMap.add oid_from (bids, pty_body, Opaque oid_to)
        in
        (quant_to, subst))
      quant_from
      (OpaqueIDMap.empty, SubstMap.empty)
  in
  let modsig_to = modsig_from |> substitute_concrete ~cause subst in
  (quant_to, modsig_to)

and typecheck_signature ~(address : Address.t) (tyenv : Typeenv.t)
    (utsig : untyped_signature) : module_signature abstracted =
  let rng, utsigmain = utsig in
  match utsigmain with
  | SigVar signm -> (
      match tyenv |> Typeenv.find_signature signm with
      | None -> raise_error (UnboundSignatureName (rng, signm))
      | Some sentry_from ->
          let absmodsig_from = sentry_from.sig_signature in
          let address_sigvar = sentry_from.sig_address in
          let absmodsig_to =
            copy_abstract_signature ~cause:rng ~address_to:address
              absmodsig_from
          in
  ...
  | SigPath (utmod1, sigident2) -> (
      ...
      | _, ConcStructure sigr1 -> (
          ...
          match sigr1 |> SigRecord.find_signature signm2 with
          | None -> raise_error (UnboundSignatureName (rng2, signm2))
          | Some sentry2 ->
              let absmodsig2 = sentry2.sig_signature in
              let _, modsig2 = absmodsig2 in
              if opaque_occurs quant1 modsig2 then
                raise_error
                  (OpaqueIDExtrudesScopeViaSignature (rng, absmodsig2))
              else absmodsig2
  ...
  | SigFunctor (modident, utsigdom, utsigcod) -> (
      ...
      let abssigcod =
        let sname = get_space_name rngm m in
        let mentry =
          { mod_signature = sigdom; mod_name = sname; mod_doc = None }
        in
      ...
      | isigdom, ConcStructure sigr ->
          ...
          ( OpaqueIDMap.empty,
            (ISigFunctor (m, isigdom, isigcod), ConcFunctor sigftor) )
      | _ -> raise_error (SupportOnlyFirstOrderFunctor rng))
  | SigWith (utsig0, modidents, tybinds) ->
      ...
      let sigr_last =
        ...
        | _, ConcFunctor _ ->
            raise_error (NotAStructureSignature (rng_last, modsig_last))
      ...
      let subst, quant =
        tydefs
        |> List.fold_left
             (fun (subst, quant) (tynm1, tentry1) ->
               let tyid0, pkd_expected =
                 match sigr_last |> SigRecord.find_type tynm1 with
                 | None -> raise_error (UndefinedTypeName (rng, tynm1))
                 | Some tentry0 -> (
                     ...
                     | None ->
                         raise_error
                           (CannotRestrictTransparentType (rng, tynm1, tentry1))
                     )
               in
      ...
      let modsig_ret =
        modsig_ret
        |> update_subsignature
             (modidents |> List.map snd)
             (fun modsig_last ->
               match modsig_last with
               | _, ConcFunctor _ -> assert false
               | _, ConcStructure sigr_last ->
                   let sigr_last =
                     sigr_last |> add_constructor_definitions ctordefs
                   in
...
and unify_kind (rng : Range.t) (tynm : type_name) ~actual:(kd1 : kind)
    ~expected:(kd2 : kind) : unit =
  ...
  | exception Invalid_argument _ ->
      ...
      raise_error
        (InvalidNumberOfTypeArguments (rng, tynm, arity_expected, arity_actual))
  | bkddomzips ->
      let bdom =
        bkddomzips
        |> List.for_all (fun (bkd1, bkd2) -> base_kind_equal bkd1 bkd2)
      in
      if bdom && base_kind_equal bkdcod1 bkdcod2 then ()
      else raise_error (KindContradiction (rng, tynm, kd1, kd2))
...
and typecheck_binding ~(address : Address.t) (tyenv : Typeenv.t)
    (utbind : untyped_binding) :
    SigRecord.t abstracted * (ModuleAttribute.t * binding list) =
  ...
  | BindVal (attrs, External extbind) ->
      ...
      let pty =
        let pre =
          ...
          in
          let pre, _ =
            make_type_parameter_assoc pre_init extbind.ext_type_params
          in
          { pre with level = 1 }
          |> add_local_row_parameter extbind.ext_row_params
        in
        let ty = decode_manual_type pre mty in
      ...
      let gname =
        ...
        generate_global_name ~is_test_suite ~arity ~has_option rngv x
      in
      let sigr =
        let ventry = { val_type = pty; val_global = gname; val_doc = None } in
  ...
  | BindVal (attrs, Internal rec_or_nonrec) ->
      ...
      let sigr, i_rec_or_nonrec =
        ...
        | Rec valbinds ->
            ...
            let recbinds =
              typecheck_letrec_mutual
                (global_name_scheme is_test_suite)
                proj pre_init valbinds
            in
            let sigr, irecbindacc =
              recbinds
              |> List.fold_left
                   (fun (sigr, irecbindacc) (x, pty, gname_outer, _, e) ->
                     let ventry =
                       {
                         val_type = pty;
                     ...
                     let irecbindacc =
                       Alist.extend irecbindacc (x, gname_outer, pty, e)
                     in
                     (sigr, irecbindacc))
        ...
        | NonRec valbind ->
            let pty, gname, e =
              let arity =
                List.length valbind.vb_parameters
                + List.length valbind.vb_mandatories
              in
              let has_option = List.length valbind.vb_optionals > 0 in
              let gnamef =
                generate_global_name ~is_test_suite ~arity ~has_option
              in
            ...
            let sigr =
              let ventry =
                { val_type = pty; val_global = gname; val_doc = None }
              in
              SigRecord.empty |> SigRecord.add_value x ventry
            in
      ...
      ((OpaqueIDMap.empty, sigr), (ModuleAttribute.empty, ibinds))
  | BindType [] -> assert false
  | BindType (_ :: _ as tybinds) ->
      let tydefs, ctordefs = bind_types ~address tyenv tybinds in
      let sigr =
        tydefs
        |> List.fold_left
             (fun sigr (tynm, tentry) -> sigr |> SigRecord.add_type tynm tentry)
             SigRecord.empty
      in
  ...
  | BindModule (modident, utsigopt2, utmod1) ->
      ...
      let sname = get_space_name rngm m in
      let mentry =
        { mod_signature = modsig; mod_name = sname; mod_doc = None }
      in
      let sigr = SigRecord.empty |> SigRecord.add_module m mentry in
      let ibinds =
  ...
  | BindInclude utmod -> (
      ...
      | _, ConcStructure sigr -> ((quant, sigr), (attrs, ibinds)))
  | BindSig (sigident, sigbind) ->
      let _, signm = sigident in
      let absmodsig = typecheck_signature ~address:Address.root tyenv sigbind in
      let sigr =
        let sentry =
          { sig_signature = absmodsig; sig_doc = None; sig_address = address }
        in
        SigRecord.empty |> SigRecord.add_signature signm sentry
      in
      ((OpaqueIDMap.empty, sigr), (ModuleAttribute.empty, []))

and bind_types ~(address : Address.t) (tyenv : Typeenv.t)
    (tybinds : type_binding list) :
    (type_name * type_entry) list * variant_definition list =
  let pre_init =
    {
      level = 0;
      tyenv;
  ...
  let synacc, vntacc, vertices, graph, tyenv =
    tybinds
    |> List.fold_left
         (fun (synacc, vntacc, vertices, graph, tyenv)
              (tyident, tyvars, syn_or_vnt) ->
           let rng, tynm = tyident in
           let kd =
             let bkddoms =
               tyvars
               |> List.map (fun (_, kdannot) ->
                      match kdannot with
                      | None -> TypeKind
           ...
           | BindVariant vntbind ->
               ...
               let tentry =
                 let bids_temp, pty_body_temp =
                   TypeConv.make_opaque_type_scheme_from_base_kinds bkds tyid
                 in
                 {
               ...
               let vntacc =
                 Alist.extend vntacc (tyident, tyvars, vntbind, tyid, kd, tentry)
               in
               (synacc, vntacc, vertices, graph, tyenv))
         ( Alist.empty,
           Alist.empty,
           SynonymNameSet.empty,
           DependencyGraph.empty,
           tyenv )
  in
  let pre = { pre_init with tyenv } in
  ...
  let graph =
    synacc |> Alist.to_list
    |> List.fold_left
         (fun graph syn ->
           let tyident, mtyreal = syn in
           let _, tynm = tyident in
           let dependencies =
             get_dependency_on_synonym_types vertices pre mtyreal
           in
           graph
           |> SynonymNameSet.fold
                (fun tynm_dep graph ->
                  graph
                  |> DependencyGraph.add_edge ~depended:tynm_dep ~depending:tynm)
                dependencies)
         graph
  in
  ...
  let tyenv, tydefacc =
    ...
    |> List.fold_left
         (fun (tyenv, tydefacc) syn ->
           ...
           let bids =
             typaramassoc |> TypeParameterAssoc.values
             |> List.map MustBeBoundID.to_bound
           in
  ...
  let tydefacc, ctordefacc =
    vntacc |> Alist.to_list
    |> List.fold_left
         (fun (tydefacc, ctordefacc) vnt ->
           ...
           let tentry =
             ...
             in
             let pty_body =
               TypeConv.substitute_poly_type bidmap pty_body_temp
             in
             { tentry with type_scheme = (bids, pty_body, Variant ctormap) }
           in
           let tydefacc = Alist.extend tydefacc (tynm, tentry) in
           let ctordefacc =
             Alist.extend ctordefacc (tynm, tyid, bids, ctormap)
           in
           (tydefacc, ctordefacc))
...
and typecheck_module ~(address : Address.t) (tyenv : Typeenv.t)
    (utmod : untyped_module) :
    module_signature abstracted * (ModuleAttribute.t * binding list) =
  ...
  | ModBinds (attrs, openspecs, utbinds) ->
      ...
      warnings |> List.iter Logging.warn_invalid_attribute;
      let tyenv = tyenv |> add_open_specs_to_type_environment openspecs in
      let abssigr, (modattr_included, ibinds) =
        typecheck_binding_list ~address tyenv utbinds
      in
...
*)
        let mentry =
          { mod_signature = modsigdom; mod_name = sname; mod_doc = None }
        in
        let tyenv = tyenv |> Typeenv.add_module m mentry in
        let address = address |> Address.append_functor_body ~arg:m in
      ...
      | _, ConcFunctor sigftor1 -> (
          ...
          | Some (modident0, utmodC, tyenv0) ->
              ...
              let subst =
                ...
                subtype_signature ~cause:rng2 ~address modsig2
                  (quant, modsigdom1)
              in
              ...
              let quant1subst, modsigcod1subst =
                absmodsigcod1 |> substitute_abstract ~cause:rng subst
              in
              let absmodsig =
                (quant1subst, copy_closure modsig0 modsigcod1subst)
              in
              (absmodsig, ibinds)))
  | ModCoerce (modident1, utsig2) ->
      ...
      let absmodsig =
        coerce_signature ~cause:rng1 ~address modsig1 absmodsig2
      in
      (absmodsig, (ModuleAttribute.empty, []))

and typecheck_binding_list ~(address : Address.t) (tyenv : Typeenv.t)
    (utbinds : untyped_binding list) :
    SigRecord.t abstracted * (ModuleAttribute.t * binding list) =
  let _tyenv, quantacc, sigracc, (modattracc, ibindacc) =
    utbinds
    |> List.fold_left
         (fun (tyenv, quantacc, sigracc, (modattracc, ibindacc)) utbind ->
           let abssigr, (modattr, ibinds) =
             typecheck_binding ~address tyenv utbind
           in
           let quant, sigr = abssigr in
           let tyenv =
             tyenv |> update_type_environment_by_signature_record sigr
           ...
           let sigracc =
             ...
             | Error s ->
                 let rng, _ = utbind in
                 raise_error (ConflictInSignature (rng, s))
             (* In the original paper "F-ing modules" [Rossberg, Russo & Dreyer 2014],
         ...
         ( tyenv,
           OpaqueIDMap.empty,
           SigRecord.empty,
           (ModuleAttribute.empty, Alist.empty) )
  in
  ((quantacc, sigracc), (modattracc, Alist.to_list ibindacc))

and coerce_signature ~(cause : Range.t) ~(address : Address.t)
    (modsig1 : module_signature) (absmodsig2 : module_signature abstracted) =
  let _subst = subtype_signature ~cause ~address modsig1 absmodsig2 in
  let quant2, modsig2 = absmodsig2 in
  (quant2, copy_closure modsig1 modsig2)

let main (tyenv : Typeenv.t) (modident : module_name ranged)
    (absmodsigopt2 : module_signature abstracted option)
    (utmod1 : untyped_module) :
    Typeenv.t
    * (signature_source * SigRecord.t) abstracted
    * space_name
    * (ModuleAttribute.t * binding list) =
  ...
  let quant, modsig =
    ...
    | Some absmodsig2 ->
        let _, modsig1 = absmodsig1 in
        coerce_signature ~cause:rng ~address modsig1 absmodsig2
  in
  match modsig with
  | _, ConcFunctor _ ->
      let rng, _ = utmod1 in
