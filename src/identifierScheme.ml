...
type t = { fragments : string list; original : string }

let is_digit ch = '0' <= ch && ch <= '9'

let is_lowercase ch = 'a' <= ch && ch <= 'z'

let is_uppercase ch = 'A' <= ch && ch <= 'Z'

let to_lowercase ch = Char.chr (Char.code ch + 0x20)
...
let from_upper_camel_case (original : string) : t option =
  ...
  try
    ...
    if is_uppercase ch0 then aux [] [ to_lowercase ch0 ] 1 else None
  with Invalid_argument _ -> None

let original (name : t) : string = name.original

let to_snake_case (name : t) : string = name.fragments |> String.concat "_"

let capitalize (is_lower_first : bool) (fragment : string) =
  if is_lower_first then fragment else String.capitalize_ascii fragment

let camel_case (is_lower : bool) (name : t) : string =
  let rec aux is_lower_first acc = function
    | [] -> List.rev acc
    | [ x ] -> List.rev (capitalize is_lower_first x :: acc)
    | x :: (y :: _ as rest) ->
        let xcap = capitalize is_lower_first x in
        if is_digit (String.get y 0) then aux false ((xcap ^ "_") :: acc) rest
        else aux false (xcap :: acc) rest
  in
...
let pp ppf name = Format.fprintf ppf "<\"%s\">" (to_snake_case name)

let compare name1 name2 = String.compare name1.original name2.original
