...
and ('a, 'b) typ_main =
  ...
  | PackType of module_signature abstracted
      [@printer
        fun ppf (qt, modsig) ->
          Format.fprintf ppf "PackType(%a, _)" pp_opaque_id_quantifier qt]
...
and functor_signature = {
  opaques : quantifier; [@printer pp_opaque_id_quantifier]
  domain : functor_domain;
  codomain : module_signature abstracted;
      [@printer
        fun ppf (qt, modsig) ->
          Format.fprintf ppf "(%a, _)" pp_opaque_id_quantifier qt]
...
and env_value_entry = { typ : poly_type; name : name; mutable is_used : bool }
...
and constructor_map =
  ((ConstructorID.t * poly_type list) ConstructorMap.t
...
and opaque_entry = { opaque_kind : kind }
  ...
  [@printer
    fun ppf acc ->
      Format.fprintf ppf "%a"
        (Format.pp_print_list pp_record_signature_entry)
        (Alist.to_list acc)])
...
and quantifier = (kind OpaqueIDMap.t[@printer pp_opaque_id_quantifier])
...
module Typeenv = struct
  ...
  let is_val_properly_used (x : identifier) (tyenv : t) : bool option =
    tyenv.values |> ValNameMap.find_opt x
    |> Option.map (fun entry -> entry.is_used)

  let fold_value f tyenv acc =
    ValNameMap.fold (fun x entry acc -> f x entry.typ acc) tyenv.values acc

  let add_constructor (ctornm : constructor_name)
      (ctorentry : constructor_entry) (tyenv : t) : t =
    {
      tyenv with
  ...
  let add_opaque_id (tynm : type_name) (oid : TypeID.t) (kd : kind) (tyenv : t)
      : t =
    { tyenv with opaques = tyenv.opaques |> OpaqueIDMap.add oid kd }
  ...
  let add_module (modnm : module_name) (mentry : module_entry) (tyenv : t) : t =
    { tyenv with modules = tyenv.modules |> ModuleNameMap.add modnm mentry }

  let find_module (modnm : module_name) (tyenv : t) : module_entry option =
    tyenv.modules |> ModuleNameMap.find_opt modnm

  let add_signature (signm : signature_name) (sentry : signature_entry)
      (tyenv : t) : t =
    {
      tyenv with
  ...
  let find_signature (signm : signature_name) (tyenv : t) :
      signature_entry option =
...
module SigRecord = struct
  ...
  let add_constructor (ctornm : constructor_name) (centry : constructor_entry)
      (sigr : t) : t =
    Alist.extend sigr (SRCtor (ctornm, centry))

  let find_constructor (ctornm0 : constructor_name) (sigr : t) :
      constructor_entry option =
    sigr |> Alist.to_rev_list
    |> List.find_map (function
         | SRCtor (ctornm, centry) ->
             if String.equal ctornm ctornm0 then Some centry else None
         | _ -> None)
  ...
  let find_dummy_fold (tynm0 : type_name) (sigr : t) : poly_type option =
    ...
    |> List.find_map (function
         | SRFold (tynm, pty) ->
             if String.equal tynm tynm0 then Some pty else None
         | _ -> None)
  ...
  let find_module (modnm0 : module_name) (sigr : t) : module_entry option =
    ...
    |> List.find_map (function
         | SRModule (modnm, mentry) ->
             if String.equal modnm modnm0 then Some mentry else None
         | _ -> None)
  ...
  let find_signature (signm0 : signature_name) (sigr : t) :
      signature_entry option =
    sigr |> Alist.to_list
    |> List.find_map (function
         | SRSig (signm, sentry) ->
             if String.equal signm signm0 then Some sentry else None
         | _ -> None)

  let fold (type a) ~v:(fv : identifier -> value_entry -> a -> a)
      ...
      ~s:(fs : signature_name -> signature_entry -> a -> a) (init : a)
      (sigr : t) : a =
    sigr |> Alist.to_list
    |> List.fold_left
         (fun acc entry ->
  ...
  let map_and_fold (type a)
      ...
      ~c:
        (fc :
          constructor_name -> constructor_entry -> a -> constructor_entry * a)
  ...
  let map (type a) ~v:(fv : identifier -> value_entry -> value_entry)
      ...
      ~s:(fs : signature_name -> signature_entry -> signature_entry) (sigr : t)
      : t =
    let sigr, () =
      sigr
      |> map_and_fold
...
*)
  ...
  let disjoint_union (sigr1 : t) (sigr2 : t) : (t, string) result =
    let check_none s opt =
      match opt with None -> () | Some _ -> raise (Conflict s)
    in
    try
      let sigr =
        sigr2 |> Alist.to_list
        |> List.fold_left
             (fun sigracc entry ->
               let () =
                 ...
                 | SRCtor (ctornm, _) ->
                     check_none ctornm (find_constructor ctornm sigr1)
                 | SRFold (_, _) -> ()
                 | SRType (tynm, _) -> check_none tynm (find_type tynm sigr1)
                 | SRModule (modnm, _) ->
                     check_none modnm (find_module modnm sigr1)
                 | SRSig (signm, _) ->
                     check_none signm (find_signature signm sigr1)
               in
               Alist.extend sigracc entry)
             sigr1
      in
      Ok sigr
    with Conflict s -> Error s
end
