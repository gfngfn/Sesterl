...
let traverse_val_single (nmap : name_map) (_, gnamefun, _, ast) :
    val_binding_output =
  match ast with
...
let make_module_string ~(suffix : string) (spec : output_spec)
    (spacepath : space_name Alist.t) : string * string =
  let spaces = spacepath |> Alist.to_list in
  match spec.module_name_output_spec with
  | SingleSnake ->
      let s =
        spaces
        |> List.map OutputIdentifier.output_space_to_snake
        |> String.concat "_"
      in
      (s, s)
  | DottedCamels ->
      let s =
        spaces
        |> List.map OutputIdentifier.output_space_to_camel
        |> String.concat "."
      in
      let s = s ^ suffix in
      (s, Printf.sprintf "'%s'" s)

let rec traverse_binding_list (spec : output_spec) (sname : space_name)
    ((gmap, smap) : name_map) (spacepath : space_name Alist.t)
    (modattr : ModuleAttribute.t) (ibinds : binding list) :
    module_binding_output list * name_map =
  ...
  let omodbindacc =
    let ovalbinds =
      ...
      |> List.map (fun ibind ->
             match ibind with
             | IBindVal (INonRec valbind) ->
                 [ traverse_val_single nmap valbind ]
             | IBindVal (IRec valbinds) ->
                 valbinds |> List.map (traverse_val_single nmap)
             | IBindVal (IExternal (gname, code)) ->
                 [ OBindValExternal (gname, code) ]
             | IBindModule _ -> [])
      |> List.concat
    in
...
let stringify_base_constant (bc : base_constant) =
  ...
  | Float r ->
      if Float.is_finite r then string_of_float r ^ "0"
  ...
  | BinaryByInts ns ->
      Printf.sprintf "<<%s>>"
        (ns |> List.map string_of_int |> String.concat ", ")
...
let stringify_single (nmap : name_map) = function
  | OutputIdentifier.Local lname -> OutputIdentifier.output_local lname
  | OutputIdentifier.Global gname ->
      ...
      Printf.sprintf "(fun %s:%s/%d)" smod r.function_name arity
...
let make_mandatory_parameters (ordipats : pattern list)
    (mndipatmap : pattern LabelAssoc.t) : pattern list =
  let mndipats =
...
let rec stringify_option_decoding_operation (nmap : name_map)
    (sname_map : string) (optipatmap : (pattern * ast option) LabelAssoc.t) :
    string =
  LabelAssoc.fold
    (fun label (ipat, default) acc ->
      let spat = stringify_pattern ipat in
      let s =
        match default with
        | None ->
            Printf.sprintf "%s = %s:%s(%s, %s), " spat
              Primitives.primitive_module_name Primitives.decode_option_function
              sname_map label
        | Some ast ->
            Printf.sprintf "%s = %s:%s(%s, %s, fun() -> %s end), " spat
              Primitives.primitive_module_name
              Primitives.decode_option_function_with_default sname_map label
              (stringify_ast nmap ast)
      in
      Alist.extend acc s)
    optipatmap Alist.empty
  |> Alist.to_list |> String.concat ""

and stringify_arguments (nmap : name_map) (mrow : mono_row)
    (ordastargs : ast list) (mndargmap : ast LabelAssoc.t)
...
and stringify_ast (nmap : name_map) (ast : ast) =
  let iter = stringify_ast nmap in
  match ast with
  | IVar name -> stringify_single nmap name
  | IBaseConst bc -> stringify_base_constant bc
  | ILambda (recopt, ordipats, mndipatmap, optipatmap, ast0) ->
      ...
      if LabelAssoc.cardinal optipatmap = 0 then
        ...
        Printf.sprintf "fun%s(%s) -> %s end" srec sparamscat s0
      else
        let sparamscatcomma =
          snames |> List.map (fun s -> s ^ ", ") |> String.concat ""
        in
        let sname_map = fresh_local_symbol () in
        let sgetopts =
          stringify_option_decoding_operation nmap sname_map optipatmap
        in
        Printf.sprintf "fun%s(%s%s) -> %s%s end" srec sparamscatcomma sname_map
          sgetopts s0
      ...
      | OutputIdentifier.Local lname, _ ->
          ...
          if can_take_optional then
            if no_mandatory_argument then
              Printf.sprintf "%s(#{%s})" sname soptmap
            else Printf.sprintf "%s(%s, #{%s})" sname sargscat soptmap
          else Printf.sprintf "%s(%s)" sname sargscat
      | OutputIdentifier.Global gname, _ ->
          ...
          let sopts =
            if LabelAssoc.cardinal optargmap = 0 then ""
              ...
              *)
            else if no_mandatory_argument then Printf.sprintf "#{%s}" soptmap
            else Printf.sprintf ", #{%s}" soptmap
          in
          Printf.sprintf "%s:%s(%s%s)" smod sfun (String.concat ", " sargs)
            sopts
  ...
  | IFreezeUpdate (ast0, astargs) ->
      ...
      Printf.sprintf "begin {%s, %s, %s} = %s, {%s, %s, %s ++ [%s]} end" varM
        varF varArgs s0 varM varF varArgs (String.concat ", " sargs)
  | IRecord emap ->
  ...
  | IReceive (branches, iafter_opt) -> (
      let sbrs =
        branches
        |> List.map (stringify_receive_branch nmap)
        |> String.concat "; "
      in
      match iafter_opt with
      | None -> Printf.sprintf "receive %s end" sbrs
      | Some (ast1, ast2) ->
          let sv = fresh_local_symbol () in
          let s1 = iter ast1 in
          let s2 = iter ast2 in
          Printf.sprintf "begin %s = %s, receive %s after %s -> %s end end" sv
            s1 sbrs sv s2)
  | ITuple es ->
  ...
  | IAssert (rng, e0) ->
      ...
      Printf.sprintf "begin %s = %s, %s(<<\"%s\">>, %d) end" var s0 var
        (Range.get_file_name rng) (Range.get_start_line rng)
...
let stringify_val_binding_output : val_binding_output -> string list = function
  | OBindVal (gnamefun, ordlnames, mndnamemap, optnamemap, gmap, ast0) ->
      ...
      let sparamscatcomma =
        sparams |> List.map (fun s -> s ^ ", ") |> String.concat ""
      in
      let sname_map = fresh_local_symbol () in
      let sgetopts =
        stringify_option_decoding_operation gmap sname_map optnamemap
      in
      let s0 = stringify_ast gmap ast0 in
      if r.has_option then
        let s_without_option =
          Printf.sprintf "%s(%s) -> ?MODULE:%s(%s#{})." r.function_name
            sparamscat r.function_name sparamscatcomma
        in
        let s_with_option =
          Printf.sprintf "%s(%s%s) -> %s%s." r.function_name sparamscatcomma
            sname_map sgetopts s0
        in
        [ s_without_option; s_with_option ]
      else
        let s = Printf.sprintf "%s(%s) -> %s." r.function_name sparamscat s0 in
        [ s ]
  | OBindValExternal (_, code) -> [ code ]
  ...
  | OBindModule
      ...
      let exports =
        ...
        |> List.map (function
               ...
               else [ Printf.sprintf "%s/%d" r.function_name r.arity ])
        |> List.concat
      in
      let ss =
        ovalbinds |> List.map stringify_val_binding_output |> List.concat
      in
      let lines =
        List.concat
          [
            [ Printf.sprintf "-module(%s)." smod_atom ];
            modattr.behaviours |> StringSet.elements
            |> List.map (fun s -> Printf.sprintf "-behaviour(%s)." s);
...
let write_file (absdir_out : absolute_dir) (smod_basename : string)
    (lines : string list) : unit =
  let abspath_out =
    Core.Filename.concat absdir_out (Printf.sprintf "%s.erl" smod_basename)
  in
...
let write_module_to_file (absdir_out : absolute_dir)
    (omodbind : module_binding_output) : unit =
    ...
    List.concat
      [
        ...
        |> List.map (fun primdef ->
               let open Primitives in
               let targetdef = primdef.target in
               Printf.sprintf "%s(%s) -> %s." targetdef.target_name
...
let main (spec : output_spec) (absdir_out : absolute_dir) (nmap : name_map)
    ~package_name:(pkgnameopt : space_name option)
    ~module_name:(sname : space_name)
    ((modattr, ibinds) : ModuleAttribute.t * binding list) : name_map =
  (*
...
*)
  ...
  in
  omodbinds
  |> List.iter (fun omodbind -> write_module_to_file absdir_out omodbind);
  nmap_after
