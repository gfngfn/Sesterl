...
let collect_ids_scheme (fidht : unit FreeIDHashTable.t)
    (fridht : LabelSet.t FreeRowIDHashTable.t) (bidht : unit BoundIDHashTable.t)
    (bridht : LabelSet.t BoundRowIDHashTable.t) =
  let aux_free_id (fid : FreeID.t) =
    if FreeIDHashTable.mem fidht fid then ()
    else FreeIDHashTable.add fidht fid ()
  in
  ...
  let aux_bound_id (bid : BoundID.t) =
    ...
    else BoundIDHashTable.add bidht bid ()
  in
  ...
  let rec aux_mono ((_, tymain) : mono_type) : unit =
    match tymain with
    | BaseType _ -> ()
    | TypeVar (Updatable { contents = Link ty }) -> aux_mono ty
    | TypeVar (Updatable { contents = Free fid }) -> aux_free_id fid
    ...
    | EffType (domain, eff, ty0) ->
        ...
        aux_mono_effect eff;
        aux_mono ty0
    | PidType pidty -> aux_mono_pid_type pidty
    | ProductType tys -> tys |> TupleList.to_list |> List.iter aux_mono
    | RecordType row -> aux_mono_row row
    | TypeApp (tyid, tyargs) -> tyargs |> List.iter aux_mono
    | PackType _absmodsig -> ()
  ...
  and aux_poly ((_, ptymain) : poly_type) : unit =
    ...
    | EffType (pdomain, peff, pty0) ->
        ...
        aux_poly pty0
    | PidType ppidty -> aux_poly_pid_type ppidty
    | ProductType ptys -> ptys |> TupleList.to_list |> List.iter aux_poly
    | RecordType prow -> aux_poly_row prow
    | TypeApp (tyid, ptyargs) -> ptyargs |> List.iter aux_poly
    | PackType _absmodsig -> ()
  (* TODO: traverse signatures *)
  ...
  and aux_mono_effect (Effect ty) = aux_mono ty
  and aux_poly_effect (Effect pty) = aux_poly pty
  and aux_mono_pid_type (Pid ty) = aux_mono ty
  and aux_poly_pid_type (Pid pty) = aux_poly pty
  and aux_mono_row : mono_row -> unit = function
    | RowCons (_rlabel, ty, row) ->
        aux_mono ty;
        aux_mono_row row
  ...
  and aux_poly_row : poly_row -> unit = function
    ...
    | RowVar (MonoRow prv) -> (
        ...
        | MustBeBoundRow _ -> ())
    | RowVar (BoundRow brid) -> aux_bound_row_id brid
    | RowEmpty -> ()
  in
  (aux_mono, aux_poly)
...
let collect_ids_mono (ty : mono_type) (dispmap : DisplayMap.t) : DisplayMap.t =
  ...
  aux_mono ty;
  let dispmap =
    FreeIDHashTable.fold
      (fun fid () dispmap -> dispmap |> DisplayMap.add_free_id fid)
      fidht dispmap
  in
  let dispmap =
    FreeRowIDHashTable.fold
      (fun frid labset dispmap ->
...
let collect_ids_poly (pty : poly_type) (dispmap : DisplayMap.t) : DisplayMap.t =
  ...
  in
  let dispmap =
    BoundIDHashTable.fold
      (fun bid () dispmap -> dispmap |> DisplayMap.add_bound_id bid)
      bidht dispmap
  in
  let dispmap =
    BoundRowIDHashTable.fold
      (fun brid labset dispmap ->
...
let normalize_row_general : ('a, 'b) row -> ('a, 'b) normalized_row =
 fun prow ->
  let rec aux plabmap = function
    | RowCons ((_, label), pty, prow) ->
        aux (plabmap |> LabelAssoc.add label pty) prow
...
let normalize_mono_row (row : mono_row) : normalized_mono_row =
  let rec aux labmap = function
    | RowCons ((_, label), ty, row) ->
        aux (labmap |> LabelAssoc.add label ty) row
...
let lift_scheme (rngf : Range.t -> Range.t) (levpred : int -> bool)
    (ty : mono_type) : poly_type =
  ...
  and aux ((rng, tymain) : mono_type) : poly_type =
    ...
    | PackType absmodsig -> (rngf rng, PackType absmodsig)
  ...
  and aux_row : mono_row -> poly_row = function
    ...
    | RowVar (MustBeBoundRow mbbrid) ->
        ...
        else RowVar (MonoRow (MustBeBoundRow mbbrid))
    | RowEmpty -> RowEmpty
  in
...
let generalize (lev : int) (ty : mono_type) : poly_type =
  lift_scheme (fun _ -> Range.dummy "erased") (fun levx -> lev < levx) ty
...
let instantiate_scheme :
      'a 'b.
      (Range.t -> poly_type_var -> ('a, 'b) typ) ->
      (poly_row_var -> 'b) ->
      poly_type ->
      ('a, 'b) typ =
 fun intern intern_row pty ->
  let rec aux (rng, ptymain) =
    ...
    | RecordType prow ->
        ...
        (rng, RecordType row)
    | TypeApp (tyid, ptyargs) -> (rng, TypeApp (tyid, ptyargs |> List.map aux))
    | PackType absmodsig -> (rng, PackType absmodsig)
  and aux_row = function
    | RowCons (rlabel, pty, prow) ->
        let ty = aux pty in
        let row = aux_row prow in
        RowCons (rlabel, ty, row)
    | RowVar prv -> RowVar (intern_row prv)
    | RowEmpty -> RowEmpty
...
let instantiate_by_hash_table bidht bridht (lev : int) (pty : poly_type) :
    mono_type =
  let rec intern (rng : Range.t) (ptv : poly_type_var) : mono_type =
    match ptv with
    | Mono mtv -> (rng, TypeVar mtv)
    | Bound bid ->
        let mtv =
          match BoundIDHashTable.find_opt bidht bid with
          | Some mtvu -> Updatable mtvu
          | None ->
  ...
  and intern_row (prv : poly_row_var) : mono_row_var =
    ...
    | BoundRow brid -> (
        ...
        | None ->
            ...
            UpdatableRow mrvu)
  and aux pty = instantiate_scheme intern intern_row pty in
  aux pty
...
let make_bound_to_free_hash_table bidht bridht (lev : int)
    (typarams : BoundID.t list) : mono_type list =
  let tyargacc =
    typarams
    |> List.fold_left
         (fun tyargacc bid ->
           let mtv =
             ...
             | None ->
                 let fid =
                   FreeID.fresh ~message:"make_bound_to_free_hash_table" lev
                 in
                 let mtvu = ref (Free fid) in
                 BoundIDHashTable.add bidht bid mtvu;
                 Updatable mtvu
...
let instantiate_type_arguments (lev : int) (typarams : BoundID.t list)
    (ptys : poly_type list) : mono_type list * mono_type list =
  let bidht = BoundIDHashTable.create 32 in
  let bridht = BoundRowIDHashTable.create 32 in
  let tyargs = make_bound_to_free_hash_table bidht bridht lev typarams in
  let tys_expected =
    ptys |> List.map (instantiate_by_hash_table bidht bridht lev)
  in
  (tyargs, tys_expected)

let substitute_mono_type (substmap : mono_type BoundIDMap.t) :
    poly_type -> mono_type =
...
let substitute_poly_type (substmap : poly_type BoundIDMap.t) :
    poly_type -> poly_type =
  let intern (rng : Range.t) (ptv : poly_type_var) : poly_type =
    match ptv with
    | Mono _ -> (rng, TypeVar ptv)
    | Bound bid -> (
        match substmap |> BoundIDMap.find_opt bid with
        | None -> assert false
        | Some pty -> pty)
...
let apply_type_scheme_mono ((bids, pty_body) : type_scheme)
    (tyargs : mono_type list) : mono_type option =
  try
    let substmap =
      List.fold_left2
        (fun substmap bid tyarg -> substmap |> BoundIDMap.add bid tyarg)
        BoundIDMap.empty bids tyargs
    in
    Some (substitute_mono_type substmap pty_body)
  with Invalid_argument _ -> None

let apply_type_scheme_poly ((bids, pty_body) : type_scheme)
    (ptyargs : poly_type list) : poly_type option =
  try
    let substmap =
      List.fold_left2
        (fun substmap bid ptyarg -> substmap |> BoundIDMap.add bid ptyarg)
        BoundIDMap.empty bids ptyargs
    in
    Some (substitute_poly_type substmap pty_body)
  with Invalid_argument _ -> None

let make_opaque_type_scheme (bids : BoundID.t list) (tyid : TypeID.t) :
    type_scheme =
...
let make_opaque_type_scheme_from_base_kinds (bkds : base_kind list)
    (tyid : TypeID.t) : type_scheme =
  let bids = bkds |> List.map (fun _bkd -> BoundID.fresh ()) in
  make_opaque_type_scheme bids tyid

let get_opaque_type ((bids, pty_body, _) : type_scheme_with_entity) :
    TypeID.t option =
  match pty_body with
...
let overwrite_range_of_type (rng : Range.t) (_, tymain) = (rng, tymain)
...
let rec arity_of_kind = function Kind (bkddoms, _) -> List.length bkddoms

(* Omit redundant structures of the given type. *)
let rec canonicalize_root = function
  | _, TypeVar (Updatable { contents = Link ty }) -> canonicalize_root ty
  | ty -> ty
...
let rec show_label_assoc :
          ...
          ('a -> string) ->
          ('b -> string option) ->
          ('a, 'b) typ LabelAssoc.t ->
          string option =
 fun ~prefix ~suffix spec seen_from showtv showrv labmap ->
  if LabelAssoc.cardinal labmap = 0 then None
  else
    let s =
      LabelAssoc.fold
        (fun label ty acc ->
...
and show_domain :
 ...
 fun spec seen_from showtv showrv domain ->
  let sdoms =
    domain.ordered |> List.map (show_type spec seen_from showtv showrv)
  in
  let sdomscat = String.concat ", " sdoms in
  let is_ord_empty = sdoms = [] in
  let is_mnds_empty, smnds =
    match
      show_label_assoc ~prefix:"-" ~suffix:"" spec seen_from showtv showrv
        domain.mandatory
    with
    | None -> (true, "")
    | Some s -> (false, s)
  ...
  let is_opts_empty, sopts =
    match
      show_row ~prefix:"?" ~suffix:"" spec seen_from showtv showrv
        domain.optional
    with
    | None -> (true, "")
    | Some s -> (false, s)
  ...
  let smid1 =
    ...
    else if is_mnds_empty && is_opts_empty then ""
    else ", "
  in
  let smid2 =
    if is_mnds_empty || is_opts_empty then ""
    else if is_ord_empty then ""
    else ", "
  in
  Printf.sprintf "%s%s%s%s%s" sdomscat smid1 smnds smid2 sopts
 ...
 fun spec seen_from showtv showrv ty ->
  let rec aux (_, tymain) =
    ...
    | FuncType (domain, tycod) ->
        ...
        Printf.sprintf "%s%s %s %s" (spec.token "fun") (spec.paren sdom)
          spec.arrow scod
    | EffType (domain, eff, ty0) ->
        let sdom = show_domain spec seen_from showtv showrv domain in
        let seff = aux_effect eff in
        let s0 = aux ty0 in
        Printf.sprintf "%s%s %s %s%s" (spec.token "fun") (spec.paren sdom)
          spec.arrow seff s0
    | PidType pidty ->
        let spid = aux_pid_type pidty in
        Printf.sprintf "pid%s" (spec.angle spid)
    | TypeVar tv -> showtv tv
    | ProductType tys ->
        let ss = tys |> TupleList.to_list |> List.map aux in
        Printf.sprintf "{%s}" (String.concat ", " ss)
    | RecordType row -> (
        match
          show_row ~prefix:"" ~suffix:" :" spec seen_from showtv showrv row
        with
        | None -> "{}"
    ...
    | TypeApp (tyid, tyargs) -> (
        ...
        | _ :: _ ->
            ...
            Format.asprintf "%a%s" (TypeID.pp ~seen_from) tyid
              (spec.angle (String.concat ", " ss)))
    | PackType _absmodsig -> "(signature)"
...
and show_row :
 ...
 fun ~prefix ~suffix spec seen_from showtv showrv row ->
  ...
  let smain_opt =
    labmap |> show_label_assoc ~prefix ~suffix spec seen_from showtv showrv
  in
  let svar_opt = match rowvar_opt with Some rv -> showrv rv | None -> None in
...
and show_mono_type_var (spec : display_spec) (seen_from : Address.t)
    (dispmap : DisplayMap.t) (mtv : mono_type_var) : string =
  match mtv with
...
and show_mono_type_var_updatable (spec : display_spec) (seen_from : Address.t)
    (dispmap : DisplayMap.t) (mtvu : mono_type_var_updatable) : string =
  match mtvu with
  | Link ty ->
      show_type spec seen_from
        (show_mono_type_var spec seen_from dispmap)
        (show_mono_row_var spec seen_from dispmap)
        ty
  | Free fid -> dispmap |> DisplayMap.find_free_id fid

and show_mono_row_var (spec : display_spec) (seen_from : Address.t)
    (dispmap : DisplayMap.t) (mrv : mono_row_var) : string option =
  match mrv with
  | UpdatableRow mrvu ->
      show_mono_row_var_updatable spec seen_from dispmap !mrvu
  | MustBeBoundRow mbbrid ->
      Some (Format.asprintf "%a" MustBeBoundRowID.pp_rich mbbrid)

and show_mono_row_var_updatable (spec : display_spec) (seen_from : Address.t)
    (dispmap : DisplayMap.t) (mrvu : mono_row_var_updatable) : string option =
  match mrvu with
  | LinkRow row ->
      show_row ~prefix:"?" ~suffix:"" spec seen_from
        (show_mono_type_var spec seen_from dispmap)
        (show_mono_row_var spec seen_from dispmap)
        row
  | FreeRow frid ->
...
let show_mono_type ?(spec : display_spec = display_spec_tty)
    ?(seen_from : Address.t = Address.root) (dispmap : DisplayMap.t) :
    mono_type -> string =
  show_type spec seen_from
    (show_mono_type_var spec seen_from dispmap)
    (show_mono_row_var spec seen_from dispmap)

let show_mono_row ~(prefix : string) ~(suffix : string)
    ?(spec : display_spec = display_spec_tty)
    ?(seen_from : Address.t = Address.root) (dispmap : DisplayMap.t) :
    mono_row -> string option =
  show_row ~prefix ~suffix spec seen_from
    (show_mono_type_var spec seen_from dispmap)
    (show_mono_row_var spec seen_from dispmap)
...
let pp_mono_row ?(spec : display_spec = display_spec_tty) dispmap ppf row =
  Format.fprintf ppf "%s"
    (Option.value ~default:"(empty)"
       (show_mono_row ~prefix:"" ~suffix:"" ~spec dispmap row))

let rec show_poly_type_var (spec : display_spec) (seen_from : Address.t)
    (dispmap : DisplayMap.t) = function
  | Bound bid -> dispmap |> DisplayMap.find_bound_id bid
  | Mono mtv -> show_mono_type_var spec seen_from dispmap mtv

and show_poly_row_var (spec : display_spec) (seen_from : Address.t)
    (dispmap : DisplayMap.t) = function
  | BoundRow brid -> Some (dispmap |> DisplayMap.find_bound_row_id brid)
  | MonoRow mrv -> show_mono_row_var spec seen_from dispmap mrv

and show_poly_type ?(spec : display_spec = display_spec_tty)
    ?(seen_from : Address.t = Address.root) (dispmap : DisplayMap.t) :
    poly_type -> string =
  show_type spec seen_from
    (show_poly_type_var spec seen_from dispmap)
    (show_poly_row_var spec seen_from dispmap)

let show_poly_row ?(spec : display_spec = display_spec_tty)
    ?(seen_from : Address.t = Address.root) (dispmap : DisplayMap.t) :
    poly_row -> string option =
  show_row ~prefix:"" ~suffix:"" spec seen_from
    (show_poly_type_var spec seen_from dispmap)
    (show_poly_row_var spec seen_from dispmap)

let pp_poly_type ?(spec : display_spec = display_spec_tty)
    ?(seen_from : Address.t = Address.root) (dispmap : DisplayMap.t)
    (ppf : Format.formatter) (pty : poly_type) : unit =
...
let show_bound_type_ids (dispmap : DisplayMap.t) =
  ...
  |> DisplayMap.fold_bound_id
       (fun bid sb acc -> Alist.extend acc (Printf.sprintf "%s :: o" sb))
       Alist.empty
  |> Alist.to_rev_list
...
let show_base_kind (bkd : base_kind) : string =
  ...
  | TypeKind -> "o"
  | RowKind labset ->
      Printf.sprintf "(%s)" (labset |> LabelSet.elements |> String.concat ", ")
...
let pp_debug_poly_type ~(raw : bool) (ppf : Format.formatter) (pty : poly_type)
    : unit =
  let dispmap =
    if raw then DisplayMap.empty else DisplayMap.empty |> collect_ids_poly pty
  in
  let ss1 = show_bound_type_ids dispmap in
  let ss2 = show_bound_row_ids dispmap in
  let s3 = show_poly_type dispmap pty in
  Format.fprintf ppf "<%s> <%s> %s" (String.concat ", " ss1)
    (String.concat ", " ss2) s3

let pp_debug_mono_type ~(raw : bool) (ppf : Format.formatter) (ty : mono_type) :
    unit =
  let dispmap =
    if raw then DisplayMap.empty else DisplayMap.empty |> collect_ids_mono ty
  in
