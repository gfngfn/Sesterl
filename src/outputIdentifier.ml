type space = ReprSpace of { number : int; main : IdentifierScheme.t }

type local =
  | ReprLocal of { number : int; hint : IdentifierScheme.t option }
  | ReprUnused
...
type global =
  ...
  | ReprDummy of { number : int }

type operator = ReprOperator of string
...
let space_of_module_name (s : string) : space option =
  ...
  IdentifierScheme.from_upper_camel_case s
  |> Option.map (fun space -> ReprSpace { number = n; main = space })

let space_of_package_name (s : string) : space option =
  let n = fresh_number () in
  IdentifierScheme.from_snake_case s
  |> Option.map (fun space -> ReprSpace { number = n; main = space })
...
let generate_global (s : string) ~(suffix : string) ~(arity : int)
    ~(has_option : bool) : global option =
  IdentifierScheme.from_snake_case s
  |> Option.map (fun ident ->
         let n = fresh_number () in
         ReprGlobal
           { number = n; function_name = ident; suffix; arity; has_option })

let operator (s : string) : operator = ReprOperator s

let unused : local = ReprUnused
  ...
  | ReprGlobal r ->
      {
        function_name =
          Printf.sprintf "'%s%s'"
            (r.function_name |> IdentifierScheme.to_snake_case)
            r.suffix;
        arity = r.arity;
...
let output_operator = function ReprOperator s -> s
...
let pp_local ppf = function
  | ReprLocal r -> (
      ...
      | Some ident ->
          Format.fprintf ppf "L%d%a" r.number IdentifierScheme.pp ident)
  | ReprUnused -> Format.fprintf ppf "UNUSED"

let pp_global ppf = function
  | ReprGlobal r ->
      Format.fprintf ppf "G%d%a/%d" r.number IdentifierScheme.pp r.function_name
        r.arity
  | ReprDummy r -> Format.fprintf ppf "G%d(dummy)" r.number

let pp_operator ppf = function
  | ReprOperator s -> Format.fprintf ppf "O\"%s\"" s
