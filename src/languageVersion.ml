...
let is_compatible ~(before : t) ~(after : t) =
  ...
  | 0, 0 -> before.minor = after.minor && before.patch <= after.patch
  | _ ->
...
let is_supported (specified_language_version : string) : bool =
  match
    (parse specified_language_version, parse Constants.semantic_version)
  with
  | _, None -> assert false
  | None, _ -> false
  | Some specified, Some required ->
      is_compatible ~before:specified ~after:required
